...
const SYSTEM_PROMPT = [
  ...
  "10) Em fichas antigas, os campos 'DATA EXAME PSICOLÓGICO' e 'DATA EXAME DE TIRO' são DATAS DE REALIZAÇÃO: a data em que o exame FOI FEITO. Datas passadas nesses campos são normais, esperadas e corretas. Retorne em data_realizacao_exame_psicologico e data_realizacao_exame_tiro, nunca trate como validade.",
    ...
    "20) CNPJ — LEITURA DE ALTA PRECISÃO: O CNPJ tem EXATAMENTE 14 dígitos numéricos (formato XX.XXX.XXX/XXXX-XX). REGRAS CRÍTICAS: (a) Leia cada dígito individualmente — nunca adivinhe ou complete. (b) NUNCA misture o CPF do titular (11 dígitos) com o CNPJ (14 dígitos); em documentos MEI/CCMEI o CPF aparece separadamente perto do nome do sócio. (c) O campo razao_social de MEI deve conter APENAS o nome da pessoa (ex: 'GILSON DO NASCIMENTO'), NUNCA o CPF concatenado. (d) CROSS-CHECK com nome de arquivo: se no bloco '=== NOMES DE ARQUIVO ===' houver um nome de arquivo com 14 dígitos consecutivos que pareça ser o CNPJ (ex: CCMEI-31837713000138.pdf), use esses dígitos como candidato e compare com o que você leu do documento. Se diferirem, prefira o do nome de arquivo E adicione warning 'CNPJ do OCR diverge do nome do arquivo — usando nome do arquivo. Confirmar manualmente.' (e) Se mesmo assim não tiver certeza absoluta, deixe o campo cnpj vazio e adicione warning 'CNPJ não extraído com segurança — conferir manualmente no documento CCMEI/Cartão CNPJ'.",
