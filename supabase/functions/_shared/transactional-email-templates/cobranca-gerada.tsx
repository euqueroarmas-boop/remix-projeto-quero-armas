...
interface Props {
  ...
  valor?: string; vencimento?: string; descricao?: string;
}

const Email = (props: Props) => (
  <Html lang="pt-BR" dir="ltr">
    <Head />
    <Preview>Nova cobrança disponível</Preview>
    <Body style={main}>
      <Container style={container}>
        <Section style={header}><Text style={brand}>QUERO ARMAS</Text></Section>
        <Container style={card}>
          <Heading style={h1}>Nova cobrança gerada</Heading>
          <Text style={text}>Olá{props.nome ? `, ${props.nome}` : ''},</Text>
          <Text style={text}>Sua cobrança foi gerada e está disponível para pagamento via PIX, boleto ou cartão.</Text>
          <Text style={text}><strong>Valor:</strong> R$ {props.valor}<br/><strong>Vencimento:</strong> {props.vencimento}<br/><strong>Descrição:</strong> {props.descricao}</Text>
          <Section style={{ textAlign: 'center', margin: '28px 0' }}>
            <Button style={button} href={props.linkPagamento || 'https://euqueroarmas.com.br'}>Pagar agora</Button>
          </Section>
          <Hr style={hr} />
...
export const template = {
  ...
  displayName: 'Nova cobrança gerada',
