...
const Email = (props: Props) => (
  <Html lang="pt-BR" dir="ltr">
    <Head />
    <Preview>Falha ao cobrar cartão</Preview>
    <Body style={main}>
      <Container style={container}>
        <Section style={header}><Text style={brand}>ARSENAL INTELIGENTE</Text></Section>
        <Container style={card}>
          <Heading style={h1}>Não conseguimos cobrar seu cartão</Heading>
          <Text style={text}>Olá{props.nome ? `, ${props.nome}` : ''},</Text>
          <Text style={text}>A tentativa de cobrança no seu cartão recorrente falhou. Atualize os dados para manter sua assinatura ativa.</Text>
          <Text style={text}><strong>Motivo:</strong> {props.motivo}</Text>
          <Section style={{ textAlign: 'center', margin: '28px 0' }}>
            <Button style={button} href={props.portalUrl || 'https://euqueroarmas.com.br'}>Atualizar cartão</Button>
          </Section>
          <Hr style={hr} />
...
export const template = {
  component: Email,
  subject: 'Falha ao cobrar cartão',
  displayName: 'Não conseguimos cobrar seu cartão',
