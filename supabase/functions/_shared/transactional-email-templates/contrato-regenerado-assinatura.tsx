...
const Email = (props: Props) => (
  <Html lang="pt-BR" dir="ltr">
    <Head />
    <Preview>Contrato atualizado — acesse a versão corrigida para assinatura</Preview>
    <Body style={main}>
      <Container style={container}>
        <Section style={header}><Text style={brand}>ARSENAL INTELIGENTE</Text></Section>
        <Container style={card}>
          <Heading style={h1}>Contrato atualizado para assinatura</Heading>
          <Text style={text}>Olá{props.nome ? `, ${props.nome}` : ''},</Text>
          <Text style={text}>Seu contrato foi regenerado pela equipe Quero Armas e a versão atualizada já está disponível no mesmo link de assinatura.</Text>
          <Text style={text}><strong>Contrato:</strong> {props.contrato}</Text>
          <Section style={{ textAlign: 'center', margin: '28px 0' }}>
            <Button style={button} href={props.linkAssinatura || 'https://euqueroarmas.com.br'}>Abrir contrato atualizado</Button>
          </Section>
          <Hr style={hr} />
          <Text style={note}>Assine somente a versão disponível neste link. O procedimento de assinatura digital segue pelo gov.br ou por certificado ICP-Brasil, com validade jurídica.</Text>
...
export const template = {
  component: Email,
  subject: 'Contrato atualizado para assinatura',
  displayName: 'Contrato atualizado para assinatura',
...
const note = { fontSize: '12px', color: '#555', lineHeight: '1.5', margin: '16px 0 18px', fontStyle: 'italic' as const }
