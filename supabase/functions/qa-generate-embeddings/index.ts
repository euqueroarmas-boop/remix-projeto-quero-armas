...
async function generateEmbeddings(documento_id: string) {
  ...
  while (pass < MAX_PASSES) {
    ...
    const { data: chunks } = await supabase
      ...
      .eq("embedding_status", "pendente")
      .order("ordem_chunk")
      .limit(50);

    if (!chunks || chunks.length === 0) {
      console.log(`Pass ${pass}: No more pending chunks for ${documento_id}. Total generated: ${totalGenerated}`);
      break;
    }
    ...
    for (const chunk of chunks) {
      try {
        ...
        if (!embResp.ok) {
          ...
          console.error(`Embedding API error: ${status}`);
          // For rate limits or payment issues, wait longer or stop
          if (status === 429) {
            console.log("Rate limited, waiting 5s...");
            await new Promise(r => setTimeout(r, 5000));
            continue; // retry this chunk on next pass
          }
          if (status === 402) {
            console.log("Payment required, stopping embeddings");
  ...
  // Update document status to concluido after all embeddings
