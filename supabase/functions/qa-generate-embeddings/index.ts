...
async function generateEmbeddings(documento_id: string) {
  ...
  if (!chunks || chunks.length === 0) {
    console.log("No pending chunks for", documento_id);
    return { generated: 0 };
  }
  ...
  for (const chunk of chunks) {
    try {
      ...
      if (!embResp.ok) {
        console.error("Embedding API error:", embResp.status);
      ...
      try {
        // Try to parse the array from the content
  ...
  return { generated, total: chunks.length };
}
