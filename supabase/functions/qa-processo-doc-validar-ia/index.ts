...
const TIPO_DOC_PROMPTS: Record<string, string> = {
  ...
  identidade_funcional: "Identidade Funcional emitida por órgão público (PM, PCSP, Exército, Marinha, Aeronáutica, PF, PRF, Corpo de Bombeiros, etc.). ATENÇÃO: este documento comprova VÍNCULO INSTITUCIONAL (ocupação lícita) — é avaliado na etapa de comprovação de renda/ocupação, NÃO na etapa de identidade civil. O cliente também envia separadamente um RG, CIN ou CNH. Extraia TODOS os dados visíveis: nome_completo, cpf (se houver), tipo_documento_detectado ('identidade_funcional'), numero_documento (matrícula funcional ou número de registro), orgao_emissor (corporação/instituição), cargo_funcional (se visível), data_nascimento (YYYY-MM-DD se houver), data_emissao (YYYY-MM-DD se houver), validade (YYYY-MM-DD se houver), uf_emissao. NÃO exija QR Code: identidade funcional não segue o padrão gov.br/CIN — o documento emitido pela corporação é válido por si só.",
  ...
  renda_contrato_social:
    "Contrato Social, última alteração consolidada, Requerimento de Empresário ou CCMEI (Certificado da Condição de Microempreendedor Individual). Extraia: razao_social, nome_fantasia (se houver), cnpj (apenas dígitos), data_constituicao/data_abertura (YYYY-MM-DD se houver), capital_social (número, se houver), endereco_sede, cidade_sede, uf_sede, objeto_social/atividade (resumo curto), socios (array com objetos {nome, cpf, participacao_percentual, qualificacao}, se houver). NÃO exija um campo 'nome_titular' único. Para MEI, aceite CCMEI como substituto funcional do contrato social/requerimento de empresário, nunca como substituto do Cartão CNPJ. REGRA DE PRAZO: documento CONSTITUTIVO — NÃO tem validade e NÃO exige data de emissão; jamais rejeite por data ausente. CONFERÊNCIA: nome do titular, CPF e situacao_cadastral (diferente de ATIVA reprova); CNPJ e razão social devem bater com o Cartão CNPJ e com o cadastro do cliente.",
  renda_qsa:
    "QSA (Quadro de Sócios e Administradores) emitido pela Receita Federal a partir do Cartão CNPJ. Extraia: razao_social, cnpj (apenas dígitos), data_emissao (YYYY-MM-DD se houver), socios (array {nome, cpf, qualificacao, data_entrada}), administradores (array {nome, cpf, qualificacao}). NÃO exija um campo 'nome_titular' único. Se o cliente cadastrado aparecer no QSA, registre cliente_e_socio=true em campos_complementares. REGRA: basta que o nome do cliente cadastrado conste no quadro — nomes de OUTROS sócios NÃO reprovam. O QSA sai da MESMA consulta do Cartão CNPJ: a data de emissão deve ser a mesma do Cartão CNPJ aprovado (validade = emissão + 30 dias).",
  renda_nf_empresa:
    "Nota fiscal recente emitida pela PESSOA JURÍDICA. Extraia: razao_social_emitente, cnpj_emitente (apenas dígitos), razao_social_tomador (se houver), cnpj_tomador (se houver), numero_nota, serie, data_emissao (YYYY-MM-DD), valor_total, municipio_emissao e natureza_operacao/servico. NÃO exija 'nome_titular': nota fiscal de empresa identifica a empresa por razão social/CNPJ/emitente. CONFERÊNCIA: avalie SOMENTE o EMITENTE/prestador — aprova quando o CNPJ ou a razão social batem com os dados de empresa do cadastro do cliente. Dados do TOMADOR nunca são comparados com o cadastro. Nota fiscal NÃO tem validade.",
...
${isIdentidadeFuncional ? `8. IDENTIDADE FUNCIONAL: documento que comprova VÍNCULO INSTITUCIONAL com órgão público (PM, PC, Exército, Marinha, Aeronáutica, PF, PRF, Bombeiros, etc.). NÃO é um substituto do documento civil (RG/CIN/CNH) — o cliente envia os dois separadamente. NÃO exija QR Code gov.br: identidades funcionais são emitidas pela corporação, não pelo gov.br. Extraia o máximo possível: nome_completo, cpf, numero_documento (matrícula/registro), orgao_emissor, cargo_funcional, data_nascimento, data_emissao, validade, uf_emissao. Valide que a corporação/órgão emissor corresponde ao tipo de ocupação declarado pelo cliente no cadastro.` : ""}
...
function ehDocumentoIdentidadeChecklist(tipo: any): boolean {
  const t = String(tipo || "").toLowerCase();
  // identidade_funcional é vínculo institucional (etapa 2), não identidade civil.
  if (t === "identidade_funcional") return false;
  return ["rg", "cin", "cnh", "rg_com_cpf", "documento_identidade", "identidade",
