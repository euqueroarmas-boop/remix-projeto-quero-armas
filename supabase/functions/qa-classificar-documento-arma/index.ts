    ...
    parameters: {
      ...
      properties: {
        ...
        camposExtraidos: {
          ...
          properties: {
            numero_documento: { type: "string", description: "Número principal do documento. Para COMPROVANTE_RESIDENCIA: APENAS o código de instalação/UC (mesmo valor de codigo_instalacao). NUNCA use número de Nota Fiscal, chave de acesso, código de barras, protocolo de autorização, CPF, mês/ano de referência ou número de fatura aqui." },
            codigo_instalacao: { type: "string", description: "APENAS para COMPROVANTE_RESIDENCIA: código de instalação / UC / matrícula da conta (somente dígitos, sem traços ou pontos). Procure pelos rótulos: 'Código de Instalação', 'Número UC', 'UC', 'Instalação', 'Nº de Instalação', 'Matrícula'. NÃO é o número da fatura, NÃO é o número da Nota Fiscal, NÃO é a chave de acesso." },
            ...
            tomador_nome: { type: "string", description: "NFS-e: nome / nome empresarial do TOMADOR do serviço (destinatário da nota). Copie literalmente da seção 'TOMADOR DO SERVIÇO'." },
            tomador_documento: { type: "string", description: "NFS-e: CNPJ/CPF/NIF do tomador (somente dígitos)." },
            tomador_endereco: { type: "string", description: "NFS-e: endereço completo do tomador, exatamente como impresso na seção 'TOMADOR DO SERVIÇO'." },
            tomador_cep: { type: "string", description: "NFS-e: CEP do tomador (somente dígitos)." },
            tomador_municipio: { type: "string", description: "NFS-e: município do tomador." },
            ...
            resultado_laudo: { type: "string", description: "Resultado do laudo: use EXATAMENTE 'apto' quando o avaliado for considerado apto ao manuseio de arma de fogo, ou 'inapto' quando não for. Marque a alternativa efetivamente assinalada no documento (X, ✔, negrito)." },
            clinica_nome: { type: "string", description: "Nome da clínica/instituto que emitiu o laudo" },
            clinica_endereco: { type: "string", description: "Endereço completo da clínica/consultório impresso no laudo (rua, número, complemento, bairro, CEP)" },
            clinica_cidade: { type: "string", description: "Cidade da clínica/consultório do profissional" },
            clinica_uf: { type: "string", description: "UF da clínica/consultório do profissional" },
            telefone_profissional: { type: "string", description: "Telefone de contato do profissional ou da clínica" },
...
const SYSTEM_PROMPT = [
  ...
  "REGRA CANÔNICA — VALIDADE INDETERMINADA: se o documento trouxer, no campo de validade, algo como 'INDETERM.', 'INDETERMINADA', 'PRAZO INDETERMINADO', 'VITALÍCIA' ou 'PERMANENTE', então o documento NÃO TEM PRAZO DE VENCIMENTO: retorne validade_indeterminada=true e deixe data_validade VAZIO. Nunca calcule, infira ou invente vencimento nesse caso, e nunca trate o documento como vencido.",
  ...
  "  Extrair: nome_completo, numero_documento (nº da identidade funcional), orgao_emissor (órgão/corporação), data_emissao, data_validade (se indeterminada, deixe em branco), filiacao_mae, filiacao_pai, data_nascimento, naturalidade, rg (nº do RG impresso, se houver).",
  "  Extrair TAMBÉM, obrigatoriamente quando impresso: corporacao (ex.: 'POLÍCIA MILITAR DO ESTADO DE SÃO PAULO'), posto_graduacao (POSTO OU GRADUAÇÃO — ex.: SOLDADO, CABO, SARGENTO, TENENTE, CAPITÃO, MAJOR, CORONEL, INVESTIGADOR, ESCRIVÃO, DELEGADO), quadro (ex.: QPPM, QOPM), re_matricula (RE, matrícula funcional ou nº funcional), unidade (OPM/lotação/batalhão), data_admissao (data de admissão/praça), situacao_funcional (ATIVO, INATIVO, RESERVA, REFORMADO).",
  "  Copie literalmente o que está impresso. Não deduza patente a partir de foto, uniforme ou nome. Campo não impresso volta vazio.",
  ...
  "  Extrair: nome_completo, cpf, data_nascimento, numero_documento (nº CNH), numero_registro_cnh, categoria_cnh, orgao_emissor, uf_emissor_rg, data_emissao (data de expedição), data_validade (prazo impresso na CNH; para uso como documento de identificação vale expedição + 10 anos), primeira_habilitacao, filiacao_mae, filiacao_pai, naturalidade, sexo.",
  ...
  "  numero_documento: coloque EXATAMENTE o mesmo valor de codigo_instalacao.",
  "  PROIBIDO em numero_documento e codigo_instalacao: número de Nota Fiscal (ex.: 'NOTA FISCAL Nº 021.024.229'), chave de acesso de 44 dígitos, código de barras, protocolo de autorização, CPF, mês/ano de referência, número de fatura.",
  "  Se não encontrar o campo UC/Código de Instalação com clareza, deixe AMBOS (numero_documento e codigo_instalacao) VAZIOS — não substitua por outro número.",
  ...
  "  Extrair: razao_social, cnpj, socios (todos os nomes), data_emissao. O QSA sai da MESMA consulta do Cartão CNPJ — a data de emissão é a mesma. Basta o cliente cadastrado constar entre os sócios; outros nomes NÃO reprovam.",
  ...
  "  Extrair: nome_completo (empresário), cpf, cnpj, nome_empresarial/razao_social, situacao_cadastral. O CCMEI NÃO tem data de emissão nem validade — deixe data_emissao e data_validade VAZIAS e nunca rejeite por isso. Confronte apenas nome, CPF e situação cadastral (diferente de ATIVA reprova).",
  "• CONTRATO_SOCIAL: contrato ou estatuto social, ou requerimento de empresário.",
  "  Extrair: razao_social, cnpj, nome_completo (sócio principal), situacao_cadastral. Documento CONSTITUTIVO: NÃO tem validade — deixe data_validade VAZIA e não rejeite por data de emissão ausente.",
  "• NOTA_FISCAL_AUTONOMO: NF de prestação de serviço emitida por autônomo/MEI.",
  "  Extrair: nome_completo, cpf, cnpj, razao_social, data_emissao, nf_valor.",
  "  OBRIGATÓRIO na NFS-e/DANFSe: a nota tem DUAS partes — 'EMITENTE DA NFS-e / Prestador do Serviço' e 'TOMADOR DO SERVIÇO'. Nunca misture as duas.",
  "  Do PRESTADOR extraia: cnpj, razao_social, prestador_endereco, prestador_cep, prestador_municipio.",
  "  Do TOMADOR extraia SEMPRE: tomador_nome (Nome / Nome Empresarial), tomador_documento (CNPJ/CPF/NIF), tomador_endereco, tomador_cep, tomador_municipio.",
  "  Copie os valores literalmente, sem abreviar e sem inventar. Se um campo do tomador não constar, deixe vazio.",
  ...
  "  Extrair: nome_completo (avaliado), cpf (avaliado), nome_profissional, registro_profissional (CRP), resultado_laudo ('apto' ou 'inapto', conforme a alternativa assinalada), clinica_nome, clinica_endereco, clinica_cidade, clinica_uf, telefone_profissional, data_emissao, data_validade.",
  "• LAUDO_CAPACIDADE_TECNICA: atestado de capacidade técnica assinado por instrutor credenciado.",
  "  Extrair: nome_completo (avaliado), cpf (avaliado), nome_profissional, registro_profissional, resultado_laudo ('apto' ou 'inapto'), clinica_nome, clinica_endereco, clinica_cidade, clinica_uf, telefone_profissional, data_emissao, data_validade.",
  "",
  "=== EFETIVA NECESSIDADE ===",
    ...
    if (textoPdfNativo && textoPdfNativo.trim().length >= 80) {
      if (detectarEscopoCertidao(textoPdfNativo) === "civel") {
        return json({
          ...
          camposExtraidos: {},
          justificativa:
            "Certidão CÍVEL. O processo exige a certidão CRIMINAL (antecedentes / distribuição de ações criminais). Documento recusado por escopo, sem uso de IA.",
          divergenciaComSelecaoManual: true,
          tipoSelecionadoNormalizado: normalizeTipoSelecionado(tipoSelecionado),
          recomendacao: "rejeitar",
          revisao_obrigatoria: true,
          escopo_documento: "civel",
          origemClassificacao: "trava_escopo_civel",
        });
      }
    }
