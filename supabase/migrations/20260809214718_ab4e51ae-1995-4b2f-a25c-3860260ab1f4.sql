...
CREATE OR REPLACE FUNCTION public.qa_painel_progresso_clientes()
 RETURNS TABLE(processo_id uuid, cliente_id integer, cliente_nome text, cliente_email text, servico_nome text, status text, fase text, total_docs integer, entregues integer, dispensados integer, proximo_doc text, ultima_atividade timestamp with time zone, dias_parado integer, cobrancas integer, criado_em timestamp with time zone)
 LANGUAGE sql
...
AS $function$
  WITH docs AS (
    SELECT pd.processo_id,
      COUNT(*) FILTER (WHERE COALESCE(pd.obrigatorio, true)
        AND pd.status NOT IN ('nao_aplicavel','dispensado_grupo','dispensado_por_reaproveitamento'))::int AS total,
      COUNT(*) FILTER (WHERE COALESCE(pd.obrigatorio, true) AND pd.status = 'aprovado')::int AS ok,
      COUNT(*) FILTER (WHERE COALESCE(pd.obrigatorio, true)
        AND pd.status IN ('dispensado_grupo','dispensado_por_reaproveitamento'))::int AS dispensados,
      COUNT(*) FILTER (WHERE COALESCE(pd.obrigatorio, true)
        AND pd.status NOT IN ('nao_aplicavel','dispensado_grupo','dispensado_por_reaproveitamento')
      ...
      COUNT(*) FILTER (WHERE COALESCE(pd.obrigatorio, true) AND pd.status = 'pendente'
        AND (lower(pd.tipo_documento) LIKE 'pergunta\_%' OR lower(pd.tipo_documento) = 'renda_definir_condicao'))::int AS cadastro_pendente,
      MAX(pd.data_envio) AS ultimo_envio,
      MAX(pd.updated_at) AS ultima_mudanca
  ...
  ef_calc AS (
    SELECT
      ef.processo_id,
      ef.updated_at AS efetiva_updated_at,
      11 AS passos_total,
      (
  ...
  ciencias AS (
    SELECT ci.cliente_id, MAX(ci.created_at) AS ultima_ciencia
  ...
  pend AS (
    SELECT
      ...
      CASE
        WHEN ga.prefixo IS NOT NULL
          ...
          AND ga.prefixo NOT IN ('documento','comprovante','declaracao','pergunta') THEN 0
        WHEN lower(pd.tipo_documento) LIKE 'pergunta\_%'
          OR lower(pd.tipo_documento) = 'renda_definir_condicao' THEN 1
        WHEN lower(pd.tipo_documento) IN ('cin','rg','cnh','identidade','foto_3x4')
          OR lower(COALESCE(pd.nome_documento,'') || ' ' || pd.tipo_documento) ~ '(foto 3x4|documento de identifica)' THEN 2
        WHEN lower(pd.tipo_documento) LIKE 'renda%'
          OR lower(COALESCE(pd.nome_documento,'') || ' ' || pd.tipo_documento) ~ '(nota fiscal|ocupa|contra ?cheque|mei|cnpj)' THEN 3
        WHEN lower(pd.tipo_documento) IN ('declaracao_necessidade_efetiva','comprovante_efetiva_necessidade')
          OR lower(COALESCE(pd.nome_documento,'')) LIKE '%efetiva necessidade%'
          THEN CASE WHEN COALESCE(d2.cadastro_pendente,0) = 0 OR efx.processo_id IS NOT NULL THEN 4 ELSE 8 END
        WHEN lower(COALESCE(pd.nome_documento,'') || ' ' || pd.tipo_documento) ~ '(laudo|psicol|capacidade t|habilita|instrutor|exame)' THEN 5
        WHEN lower(COALESCE(pd.nome_documento,'') || ' ' || pd.tipo_documento) ~ '(certid|antecedent)' THEN 6
        WHEN lower(COALESCE(pd.nome_documento,'') || ' ' || pd.tipo_documento) ~ '(requerimento|procura)' THEN 9
        ELSE 7
      END AS prio,
      CASE
        WHEN lower(pd.tipo_documento) LIKE 'pergunta\_%'
          OR lower(pd.tipo_documento) = 'renda_definir_condicao' THEN 'CADASTRO'
        WHEN lower(pd.tipo_documento) IN ('declaracao_necessidade_efetiva','comprovante_efetiva_necessidade')
        ...
        WHEN lower(COALESCE(pd.nome_documento,'') || ' ' || pd.tipo_documento) ~ '(certid|antecedent)' THEN 'CERTIDÕES'
    ...
    LEFT JOIN docs d2 ON d2.processo_id = pd.processo_id
    LEFT JOIN ef_calc efx ON efx.processo_id = pd.processo_id
  ...
  base AS (
    SELECT
      ...
      (COALESCE(d.ok,0)
        + CASE WHEN COALESCE(d.efetiva_docs,0) > 0
               THEN COALESCE(efc.passos_ok, 0) - COALESCE(d.efetiva_docs_ok,0) ELSE 0 END)::int AS entregues,
      COALESCE(d.dispensados,0)::int AS dispensados,
      pr.nome AS proximo_doc,
      pr.cat AS proximo_cat,
      GREATEST(
        p.created_at,
        COALESCE(d.ultimo_envio, p.created_at),
        COALESCE(d.ultima_mudanca, p.created_at),
        COALESCE(efc.efetiva_updated_at, p.created_at),
        COALESCE(ci.ultima_ciencia, p.created_at)
      ) AS ultima_atividade,
    ...
    LEFT JOIN ciencias ci ON ci.cliente_id = p.cliente_id
