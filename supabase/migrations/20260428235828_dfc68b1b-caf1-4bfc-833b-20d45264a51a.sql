CREATE OR REPLACE FUNCTION public.qa_load_staging_chunk(p_payload jsonb)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
DECLARE
  ...
  v_kind   text := p_payload->>'kind';     -- 'clientes' | 'crs' | 'senhas' | 'crafs'
...
BEGIN
  IF NOT public.qa_is_active_staff(v_uid) THEN
    RAISE EXCEPTION 'Forbidden: only QA staff may load staging.';
  END IF;
  ...
  -- Enriquece senhas com CPF/email do cliente quando aplicável
...
REVOKE ALL ON FUNCTION public.qa_load_staging_chunk(jsonb) FROM anon, authenticated;
GRANT EXECUTE ON FUNCTION public.qa_load_staging_chunk(jsonb) TO authenticated;
COMMENT ON FUNCTION public.qa_load_staging_chunk(jsonb) IS
'Loader idempotente do banco Access para staging. Restrito a staff QA ativo. Não exibe senhas em logs.';