-- Triggers verdes: documento com validade cadastrado/renovado dispara
-- qa-notify-event (motor existente) com evento 'documento_em_dia'.
-- Reusa o mesmo padrão de pg_net + Vault já em uso por
-- qa_dispatch_notify_event (20260430195234). Nenhum motor novo é criado.
...
DECLARE
  ...
  v_old_validade date;
BEGIN
  -- Descobre URL + chave (mesmo padrão da qa_dispatch_notify_event)
  BEGIN
  ...
  IF v_url IS NULL THEN
    -- Fallback: monta a partir do project_ref se disponível
    BEGIN
      SELECT decrypted_secret INTO v_url
  ...
  END IF;
  IF v_url IS NULL THEN
    -- Sem URL configurada: silencia (não bloqueia insert)
    RETURN NEW;
  END IF;
  ...
  ELSIF TG_TABLE_NAME = 'qa_documentos_cliente' THEN
    -- Só dispara se tiver validade e não for tipo já coberto por outra rotina
  ...
  -- No UPDATE só dispara se a validade AUMENTOU (renovação) — outros updates ignorados
...
EXCEPTION WHEN OTHERS THEN
  -- Nunca bloquear o insert por falha de notificação
  RETURN NEW;
END;
$$;
...
-- Documentos genéricos com validade (autorização de compra etc.)
