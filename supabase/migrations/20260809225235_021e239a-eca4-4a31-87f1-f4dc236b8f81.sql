CREATE OR REPLACE FUNCTION public.qa_painel_progresso_clientes()
 RETURNS TABLE(processo_id uuid, cliente_id integer, cliente_nome text, cliente_email text, servico_nome text, status text, fase text, total_docs integer, entregues integer, dispensados integer, reaproveitados integer, documentos_pendentes integer, perguntas_pendentes integer, em_analise integer, grupo_atual text, grupo_total integer, grupo_concluidos integer, bloqueado_por_prerequisito boolean, proximo_doc text, proximo_tipo text, ultima_atividade timestamp with time zone, dias_parado integer, cobrancas integer, criado_em timestamp with time zone)
 LANGUAGE sql
...
doc_totais AS (
  SELECT
    de.processo_id,
    count(*) FILTER (WHERE NOT (lower(coalesce(de.status,'')) IN ('dispensado','dispensado_grupo','dispensado_por_reaproveitamento','nao_aplicavel','reaproveitado','hub_reaproveitado')))::int AS docs_total,
    count(*) FILTER (WHERE de.familia='cumprido' AND NOT (lower(coalesce(de.status,'')) IN ('dispensado','dispensado_grupo','dispensado_por_reaproveitamento','nao_aplicavel','reaproveitado','hub_reaproveitado')))::int AS docs_concluidos,
    ...
    count(*) FILTER (WHERE lower(de.tipo_documento) IN ('declaracao_necessidade_efetiva','comprovante_efetiva_necessidade') AND NOT (lower(coalesce(de.status,'')) IN ('dispensado','dispensado_grupo','dispensado_por_reaproveitamento','nao_aplicavel','reaproveitado','hub_reaproveitado')))::int AS efetiva_docs,
    count(*) FILTER (WHERE lower(de.tipo_documento) IN ('declaracao_necessidade_efetiva','comprovante_efetiva_necessidade') AND de.familia='cumprido' AND NOT (lower(coalesce(de.status,'')) IN ('dispensado','dispensado_grupo','dispensado_por_reaproveitamento','nao_aplicavel','reaproveitado','hub_reaproveitado')))::int AS efetiva_docs_concluidos,
...
grupo_totais AS (
  SELECT
    pr.processo_id,
    pr.grupo_id,
    count(*) FILTER (WHERE NOT (lower(coalesce(de.status,'')) IN ('dispensado','dispensado_grupo','dispensado_por_reaproveitamento','nao_aplicavel','reaproveitado','hub_reaproveitado')))::int
    ...
    count(*) FILTER (WHERE de.familia='cumprido' AND NOT (lower(coalesce(de.status,'')) IN ('dispensado','dispensado_grupo','dispensado_por_reaproveitamento','nao_aplicavel','reaproveitado','hub_reaproveitado')))::int
