...
-- Phase 20-C: Homologation RPCs (staff only, idempotent, audit-trailed)

CREATE OR REPLACE FUNCTION public.qa_homologar_cliente(
...
BEGIN
  ...
  SELECT id, nome_completo, homologacao_status, status_cliente, cliente_legado
    INTO v_cliente
  ...
  IF v_cliente.status_cliente = 'excluido_lgpd' THEN
    RAISE EXCEPTION 'cliente_excluido_lgpd: id=%', p_cliente_id
      USING ERRCODE = '22023';
  END IF;

  -- Idempotente: já homologado
  ...
  -- Idempotência do evento: não duplicar
  SELECT EXISTS (
  ...
  IF NOT v_evt_existente THEN
    INSERT INTO public.qa_cliente_homologacao_eventos (
      cliente_id, tipo_evento, ator, user_id, descricao, dados_json
    ) VALUES (
      p_cliente_id,
      'cliente_homologado',
      'equipe_operacional',
...
REVOKE ALL ON FUNCTION public.qa_homologar_cliente(integer, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.qa_homologar_cliente(integer, text) TO authenticated;


CREATE OR REPLACE FUNCTION public.qa_reabrir_homologacao_cliente(
  p_cliente_id integer,
  p_motivo text
)
RETURNS jsonb
...
BEGIN
  ...
  SELECT id, homologacao_status, status_cliente
    INTO v_cliente
    FROM public.qa_clientes
   WHERE id = p_cliente_id;
...
REVOKE ALL ON FUNCTION public.qa_reabrir_homologacao_cliente(integer, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.qa_reabrir_homologacao_cliente(integer, text) TO authenticated;
