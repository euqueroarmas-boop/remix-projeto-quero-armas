...
GRANT SELECT ON public.qa_tipos_documento_catalogo TO authenticated;
GRANT ALL ON public.qa_tipos_documento_catalogo TO service_role;
...
GRANT SELECT ON public.qa_tipos_documento_servicos TO authenticated;
GRANT ALL ON public.qa_tipos_documento_servicos TO service_role;
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='qa_tipos_documento_catalogo' AND policyname='qa_tipos_documento_catalogo_select_authenticated') THEN
    CREATE POLICY qa_tipos_documento_catalogo_select_authenticated ON public.qa_tipos_documento_catalogo FOR SELECT TO authenticated USING (true);
  END IF;
END $$;
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='qa_tipos_documento_servicos' AND policyname='qa_tipos_documento_servicos_select_authenticated') THEN
    CREATE POLICY qa_tipos_documento_servicos_select_authenticated ON public.qa_tipos_documento_servicos FOR SELECT TO authenticated USING (true);
  END IF;
END $$;

WITH catalogo (
  tipo_documento, categoria_hub, subcategoria_hub, escopo_documental, label_publico,
  descricao_upload, aceita_ia, aceita_vinculo_arma, exige_validade, reaproveitavel_global,
  revisao_humana_obrigatoria, ordem, fonte_normativa
) AS (
  VALUES
...
SELECT tipo_documento, categoria_hub, subcategoria_hub, escopo_documental, label_publico,
  descricao_upload, aceita_ia, aceita_vinculo_arma, exige_validade, reaproveitavel_global,
  revisao_humana_obrigatoria, ordem, fonte_normativa
FROM catalogo
...
WITH compat AS (
  SELECT DISTINCT ON (sd.servico_id::bigint, lower(sd.tipo_documento))
    ...
    sd.obrigatorio, sd.ordem,
    c.escopo_documental, c.revisao_humana_obrigatoria, c.exige_validade
  FROM public.qa_servicos_documentos sd
  JOIN public.qa_tipos_documento_catalogo c ON c.tipo_documento = lower(sd.tipo_documento)
  ORDER BY sd.servico_id::bigint, lower(sd.tipo_documento), sd.ordem NULLS LAST, sd.obrigatorio DESC
)
INSERT INTO public.qa_tipos_documento_servicos (
  servico_id, tipo_documento, modo_reaproveitamento, validade_dias, obrigatorio, ordem, observacao_regra
)
SELECT servico_id, tipo_documento,
  CASE WHEN escopo_documental = 'processo' OR revisao_humana_obrigatoria THEN 'assistido'
       WHEN escopo_documental IN ('permanente', 'arma', 'cac_atividade') THEN 'automatico'
       ELSE 'assistido' END,
  CASE WHEN exige_validade THEN 365 ELSE NULL END,
  coalesce(obrigatorio, false), coalesce(ordem, 100),
  CASE WHEN escopo_documental = 'processo' THEN 'Documento contextual do processo; reaproveitamento assistido.'
       WHEN revisao_humana_obrigatoria THEN 'Requer revisão humana antes do reaproveitamento.'
       ELSE 'Compatibilidade base herdada do catálogo do serviço.' END
FROM compat
ON CONFLICT (servico_id, tipo_documento) DO UPDATE SET
...
SET categoria_hub = c.categoria_hub,
...
WHERE lower(d.tipo_documento) = c.tipo_documento
  AND ( d.categoria_hub IS DISTINCT FROM c.categoria_hub OR
...
SET categoria_hub = 'arma_acervo', escopo_documental = 'arma'
WHERE ( coalesce(arma_numero_serie, '') <> '' OR coalesce(numero_cad_sinarm, '') <> '' OR
        coalesce(numero_registro_sigma, '') <> '' OR
        lower(tipo_documento) IN ('cr', 'craf', 'sinarm', 'gt', 'gte', 'autorizacao_compra', 'nota_fiscal_arma') )
  AND ( categoria_hub IS NULL OR categoria_hub = 'outros' OR escopo_documental IS NULL );

UPDATE public.qa_documentos_cliente
SET categoria_hub = 'outros', escopo_documental = 'processo'
...
INSERT INTO public.qa_tipos_documento_catalogo (
  tipo_documento, categoria_hub, subcategoria_hub, escopo_documental, label_publico,
  descricao_upload, aceita_ia, aceita_vinculo_arma, exige_validade, reaproveitavel_global,
  revisao_humana_obrigatoria, ativo, ordem, fonte_normativa
)
VALUES
  ('declaracao_correlata', 'declaracoes', 'correlata', 'permanente', 'Declaração correlata',
   'Declaração complementar do titular para o serviço aplicável.', false, false, false, true, true, true, 54,
   ARRAY['Lei 10.826/2003', 'IN DG/PF 201', 'IN DG/PF 311']),
  ('documento_complementar_caso', 'efetiva_necessidade', 'complementar', 'processo', 'Documento complementar do caso',
   'Anexo complementar para reforço da análise do caso concreto.', false, false, false, false, true, true, 71,
   ARRAY['Lei 10.826/2003', 'IN DG/PF 201']),
  ('oficio', 'documentos_processo', 'oficio', 'processo', 'Ofício',
   'Documento processual específico do protocolo atual.', false, false, false, false, true, true, 100,
...
SET label_publico = CASE tipo_documento
      ...
      ELSE label_publico END,
    categoria_hub = CASE WHEN tipo_documento IN ('procuracao', 'recurso_administrativo_doc', 'mandado_seguranca_doc') THEN 'documentos_processo' ELSE categoria_hub END,
    subcategoria_hub = CASE WHEN tipo_documento = 'procuracao' THEN 'procuracao'
                            WHEN tipo_documento = 'recurso_administrativo_doc' THEN 'recurso'
                            WHEN tipo_documento = 'mandado_seguranca_doc' THEN 'mandado'
                            ELSE subcategoria_hub END,
    escopo_documental = CASE WHEN tipo_documento IN ('procuracao', 'recurso_administrativo_doc', 'mandado_seguranca_doc') THEN 'processo' ELSE escopo_documental END,
    reaproveitavel_global = CASE WHEN tipo_documento IN ('procuracao', 'recurso_administrativo_doc', 'mandado_seguranca_doc') THEN false ELSE reaproveitavel_global END,
    revisao_humana_obrigatoria = CASE WHEN tipo_documento IN ('declaracao_correlata', 'documento_complementar_caso', 'oficio', 'procuracao', 'recurso_administrativo_doc', 'mandado_seguranca_doc') THEN true ELSE revisao_humana_obrigatoria END,
    updated_at = now()
WHERE tipo_documento IN (
  'rg_com_cpf','comprovante_residencia','declaracao_responsavel_imovel','comprovante_efetiva_necessidade',
  'comprovante_habitualidade','comprovante_clube_tiro','comprovante_competicao','protocolo_processo',
  'despacho','exigencia','indeferimento','procuracao','recurso_administrativo_doc','mandado_seguranca_doc'
);
