...
-- 1. Ajustar dimensão do embedding para 768 (modelo google/text-embedding-004 do Lovable AI)
ALTER TABLE public.qa_kb_artigos ALTER COLUMN embedding TYPE vector(768);

-- 2. Trigger: ao alterar title/body/symptoms/tags, zerar embedding (será regerado)
CREATE OR REPLACE FUNCTION public.qa_kb_artigos_invalidate_embedding()
...
DROP TRIGGER IF EXISTS trg_qa_kb_artigos_invalidate_emb ON public.qa_kb_artigos;
CREATE TRIGGER trg_qa_kb_artigos_invalidate_emb
BEFORE UPDATE ON public.qa_kb_artigos
FOR EACH ROW EXECUTE FUNCTION public.qa_kb_artigos_invalidate_embedding();

-- 3. Índice vetorial (ivfflat com cosine)
CREATE INDEX IF NOT EXISTS idx_qa_kb_artigos_embedding
  ON public.qa_kb_artigos USING ivfflat (embedding vector_cosine_ops) WITH (lists = 50);

-- 4. RLS para cliente: pode SELECT artigos publicados e audience='cliente'
DROP POLICY IF EXISTS "kb_cliente_select" ON public.qa_kb_artigos;
CREATE POLICY "kb_cliente_select"
ON public.qa_kb_artigos
FOR SELECT
TO authenticated
USING (status = 'published' AND audience = 'cliente');

-- 5. Função híbrida (texto + tags + sintomas + vetor)
CREATE OR REPLACE FUNCTION public.qa_kb_search_hybrid(
  _query TEXT,
  _qemb vector(768) DEFAULT NULL,
