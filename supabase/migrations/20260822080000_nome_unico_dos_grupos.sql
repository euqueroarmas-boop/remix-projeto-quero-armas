-- =============================================================================
-- UM NOME SÓ PARA CADA GRUPO (22/08/2026) — decidido pelo titular.
--
-- Havia QUATRO lugares nomeando os mesmos grupos, cada um com palavras
-- próprias: o módulo do cliente/checklist (TypeScript), o campo do card do
-- painel, o campo "fase" do MESMO card, e a tabela qa_checklist_grupos.
-- "Idoneidade" aparecia como CERTIDÕES e como Antecedentes criminais; "Laudos"
-- aparecia como EXAMES e como Aptidão psicológica e técnica.
--
-- Decisão: IDONEIDADE e LAUDOS. Vocabulário único, igual ao do módulo
-- TypeScript (`pendenciasGrupos.ts`), que já era o canônico:
--
...
--   outros                   Fechamento
--
-- Três partes: (1) função-dicionário, fonte única no banco; (2) a tabela
-- qa_checklist_grupos passa a usar esses nomes, ganha os grupos que faltavam,
-- corrige a ordem (lá Antecedentes vinha ANTES de Ocupação, ao contrário do
-- resto) e aposenta o slug antigo `saude`; (3) a função do painel passa a ler
-- o dicionário em vez de repetir as palavras.
-- Nenhum documento, status ou checklist é alterado — só rótulo.
-- =============================================================================

-- ─── 1) Dicionário: fonte única dos nomes no banco ──────────────────────────
...
AS $function$
  SELECT CASE lower(btrim(COALESCE(p_slug, '')))
    ...
    WHEN 'saude'                   THEN 'Laudos'   -- slug antigo, ainda em dados velhos
    WHEN 'laudos'                  THEN 'Laudos'
    WHEN 'requerimento'            THEN 'Requerimento'
    WHEN 'outros'                  THEN 'Fechamento'
    ELSE initcap(replace(COALESCE(NULLIF(btrim(p_slug), ''), 'outros'), '_', ' '))
  END;
$function$;

COMMENT ON FUNCTION public.qa_grupo_nome(text) IS
  'Nome único de cada grupo do checklist. Espelha pendenciasGrupos.ts. '
  'Quem exibe em caixa alta aplica upper() — as palavras moram só aqui.';

GRANT EXECUTE ON FUNCTION public.qa_grupo_nome(text) TO authenticated, anon, service_role;

-- ─── 2) Tabela de grupos: nomes, ordem e slug antigo ────────────────────────
-- Aposenta o slug 'saude' (renomeado para 'laudos' em 06/08) sem apagar linha:
-- se já existir um 'laudos' no mesmo escopo, o antigo é desativado.
UPDATE public.qa_checklist_grupos g
   SET slug = 'laudos'
 WHERE g.slug = 'saude'
   AND NOT EXISTS (
     SELECT 1 FROM public.qa_checklist_grupos o
      WHERE o.slug = 'laudos' AND COALESCE(o.servico_id, 0) = COALESCE(g.servico_id, 0));

UPDATE public.qa_checklist_grupos SET ativo = false WHERE slug = 'saude';

-- Grupos que faltavam na tabela (escopo global, servico_id nulo).
INSERT INTO public.qa_checklist_grupos (slug, nome, ordem, cor)
SELECT v.slug, public.qa_grupo_nome(v.slug), v.ordem, '#7A1F2B'
  FROM (VALUES
    ('exigencias_pf', 5), ('assinaturas', 10), ('perguntas', 20),
    ('antecedentes_anteriores', 65)
  ) AS v(slug, ordem)
 WHERE NOT EXISTS (
   SELECT 1 FROM public.qa_checklist_grupos g
    WHERE g.slug = v.slug AND g.servico_id IS NULL);

-- Nome único em todas as linhas + ordem canônica no escopo global.
UPDATE public.qa_checklist_grupos g
   SET nome  = public.qa_grupo_nome(g.slug),
       ordem = CASE WHEN g.servico_id IS NULL THEN
         CASE g.slug
           WHEN 'exigencias_pf' THEN 5   WHEN 'assinaturas' THEN 10
           WHEN 'perguntas' THEN 20      WHEN 'identificacao' THEN 30
           WHEN 'endereco' THEN 40       WHEN 'ocupacao' THEN 50
           WHEN 'antecedentes' THEN 60   WHEN 'antecedentes_anteriores' THEN 65
           WHEN 'habitualidade' THEN 70  WHEN 'arma' THEN 72
           WHEN 'declaracoes' THEN 75    WHEN 'efetiva_necessidade' THEN 80
           WHEN 'laudos' THEN 90         WHEN 'requerimento' THEN 95
           WHEN 'outros' THEN 99         ELSE g.ordem
         END
       ELSE g.ordem END
 WHERE g.nome IS DISTINCT FROM public.qa_grupo_nome(g.slug)
    OR g.servico_id IS NULL;

-- ─── 3) Painel: passa a ler o dicionário em vez de repetir as palavras ──────
-- Cópia fiel da definição vigente; mudam SÓ os dois CASE de rótulo
-- (`fase` e `grupo_atual`). Nenhuma conta, filtro ou junção foi tocada.
CREATE OR REPLACE FUNCTION public.qa_painel_progresso_clientes()
 RETURNS TABLE(processo_id uuid, cliente_id integer, cliente_nome text, cliente_email text, servico_nome text, status text, fase text, total_docs integer, entregues integer, dispensados integer, reaproveitados integer, documentos_pendentes integer, perguntas_pendentes integer, em_analise integer, grupo_atual text, grupo_total integer, grupo_concluidos integer, grupos_total integer, grupo_indice integer, grupos_concluidos integer, grupos_restantes integer, bloqueado_por_prerequisito boolean, proximo_doc text, proximo_tipo text, ultima_atividade timestamp with time zone, dias_parado integer, cobrancas integer, criado_em timestamp with time zone, online boolean, ultimo_acesso timestamp with time zone, efetiva_status text, protocolo_numero text)
 LANGUAGE sql
...
ef_calc AS (
  SELECT ef.processo_id, ef.updated_at AS efetiva_updated_at, ef.ef_status, 11::int AS passos_total,
...
doc_totais AS (
  SELECT
    de.processo_id,
    count(*) FILTER (WHERE NOT (lower(coalesce(de.status,'')) IN ('dispensado_grupo','nao_aplicavel')))::int AS docs_total,
    count(*) FILTER (WHERE de.familia='cumprido' AND NOT (lower(coalesce(de.status,'')) IN ('dispensado_grupo','nao_aplicavel')))::int AS docs_concluidos,
    ...
    count(*) FILTER (WHERE lower(de.tipo_documento) IN ('declaracao_necessidade_efetiva','comprovante_efetiva_necessidade') AND NOT (lower(coalesce(de.status,'')) IN ('dispensado_grupo','nao_aplicavel')))::int AS efetiva_docs,
    count(*) FILTER (WHERE lower(de.tipo_documento) IN ('declaracao_necessidade_efetiva','comprovante_efetiva_necessidade') AND de.familia='cumprido' AND NOT (lower(coalesce(de.status,'')) IN ('dispensado_grupo','nao_aplicavel')))::int AS efetiva_docs_concluidos,
...
grupos_processo AS (
  SELECT x.*, row_number() OVER (PARTITION BY x.processo_id ORDER BY x.grupo_ordem, x.grupo_id) AS rn
  FROM (
    SELECT de.processo_id, de.grupo_id, min(de.grupo_ordem) AS grupo_ordem,
           bool_or(de.aplicavel AND de.familia='pendencia') AS tem_pendencia
    FROM docs_enriquecidos de
    WHERE de.aplicavel
      AND NOT (lower(coalesce(de.status,'')) IN ('dispensado_grupo','nao_aplicavel'))
    GROUP BY de.processo_id, de.grupo_id
  ) x
),
grupos_resumo AS (
  SELECT gp.processo_id,
    count(*)::int AS grupos_total,
    count(*) FILTER (WHERE NOT gp.tem_pendencia)::int AS grupos_concluidos,
    count(*) FILTER (WHERE gp.tem_pendencia)::int AS grupos_restantes,
    COALESCE(max(gp.rn) FILTER (WHERE gp.grupo_id = pr.grupo_id), 0)::int AS grupo_indice
  FROM grupos_processo gp
  LEFT JOIN proximo pr ON pr.processo_id = gp.processo_id
  GROUP BY gp.processo_id, pr.grupo_id
),
grupo_totais AS (
  SELECT
    pr.processo_id,
    pr.grupo_id,
    count(*) FILTER (WHERE NOT (lower(coalesce(de.status,'')) IN ('dispensado_grupo','nao_aplicavel')))::int
    ...
    count(*) FILTER (WHERE de.familia='cumprido' AND NOT (lower(coalesce(de.status,'')) IN ('dispensado_grupo','nao_aplicavel')))::int
...
acessos AS (
  SELECT lower(btrim(email)) AS email, max(created_at) AS ultimo_acesso
  FROM public.qa_cliente_login_eventos
  WHERE email IS NOT NULL
  GROUP BY lower(btrim(email))
),
protos AS (
  SELECT qa_cliente_id AS cliente_id, servico_id, max(numero) AS numero
  FROM public.qa_protocolos
  GROUP BY qa_cliente_id, servico_id
),
base AS (
  SELECT
    ...
    COALESCE(gr.grupos_total,0)::int AS grupos_total,
    COALESCE(gr.grupo_indice,0)::int AS grupo_indice,
    COALESCE(gr.grupos_concluidos,0)::int AS grupos_concluidos,
    COALESCE(gr.grupos_restantes,0)::int AS grupos_restantes,
    ...
    GREATEST(pb.created_at, COALESCE(dt.ultimo_envio,pb.created_at), COALESCE(ec.efetiva_updated_at,pb.created_at), COALESCE(ci.ultima_ciencia,pb.created_at), COALESCE(ac.ultimo_acesso,pb.created_at)) AS ultima_atividade,
    COALESCE(cb.qtd,0) AS cobrancas,
    pb.created_at AS criado_em,
    ac.ultimo_acesso,
    ec.ef_status,
    pt.numero AS protocolo_numero
  FROM processos_base pb
  ...
  LEFT JOIN grupos_resumo gr ON gr.processo_id=pb.id
  ...
  LEFT JOIN acessos ac ON ac.email = lower(btrim(pb.cliente_email))
  LEFT JOIN protos pt ON pt.cliente_id=pb.cliente_id AND pt.servico_id=pb.servico_id
)
SELECT
  b.processo_id, b.cliente_id, b.cliente_nome, b.cliente_email, b.servico_nome, b.status,
  -- RÓTULO ÚNICO: as palavras vêm de qa_grupo_nome. Os três estados especiais
  -- (bloqueio, órgão, pronto) continuam iguais.
  CASE
    ...
    WHEN b.grupo_id IS NULL THEN 'DOCUMENTOS'
    ELSE upper(public.qa_grupo_nome(b.grupo_id))
  END AS fase,
  b.total_docs,
  ...
  b.em_analise,
  upper(public.qa_grupo_nome(b.grupo_id)) AS grupo_atual,
  b.grupo_total,
  ...
  b.cobrancas,
  b.criado_em,
  (b.ultimo_acesso IS NOT NULL AND b.ultimo_acesso >= now() - interval '15 minutes') AS online,
  b.ultimo_acesso,
  b.ef_status AS efetiva_status,
  b.protocolo_numero
FROM base b
