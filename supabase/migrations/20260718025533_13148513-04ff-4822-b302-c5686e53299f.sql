CREATE OR REPLACE FUNCTION public.qa_reaproveitar_documentos_hub_processo(
  p_processo_id uuid,
...
REVOKE ALL ON FUNCTION public.qa_reaproveitar_documentos_hub_processo(uuid, text) FROM PUBLIC, anon;
...
DECLARE
  ...
  r record;
BEGIN
  IF NEW.status <> 'aprovado' AND NEW.validado_admin IS DISTINCT FROM true THEN
    RETURN NEW;
  END IF;
  IF TG_OP = 'UPDATE'
     AND (OLD.status = 'aprovado' OR OLD.validado_admin IS TRUE)
     AND NEW.arquivo_storage_path IS NOT DISTINCT FROM OLD.arquivo_storage_path
     AND NEW.data_emissao IS NOT DISTINCT FROM OLD.data_emissao
     AND NEW.data_validade IS NOT DISTINCT FROM OLD.data_validade THEN
    RETURN NEW;
  END IF;
  ...
  LOOP
    PERFORM public.qa_reaproveitar_documentos_hub_processo(r.processo_id, 'hub_documento_aprovado');
  END LOOP;
...
BEGIN
  ...
  SELECT * INTO v_reuso
    FROM public.qa_reaproveitar_documentos_hub_processo(p_processo_id, 'checklist_explodido');
