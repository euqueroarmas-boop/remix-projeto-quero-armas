-- Permite 'aguardando_assinatura' na constraint antiga (a nova já permite)
ALTER TABLE public.qa_processos DROP CONSTRAINT IF EXISTS qa_processos_status_check;
ALTER TABLE public.qa_processos ADD CONSTRAINT qa_processos_status_check CHECK (
  status = ANY (ARRAY[
...
-- RPC: ao confirmar pagamento sem contrato validated, mover processo para
-- 'aguardando_assinatura' com pagamento_status='confirmado'. NÃO explode checklist.
...
BEGIN
  ...
  -- Idempotência total: pagamento já confirmado E (já tem checklist OU já está em aguardando_assinatura)
  IF v_proc.pagamento_status = 'confirmado' AND v_proc.status <> 'aguardando_pagamento' THEN
  ...
  IF p_origem <> 'contrato_validado' AND p_bypass_contrato_validado IS NOT TRUE THEN
    ...
    IF v_has_validated_contract IS NOT TRUE THEN
      -- Confirma o pagamento e move para 'aguardando_assinatura' (não explode checklist).
      -- Arsenal gratuito segue liberado pelo frontend; checklist só após contrato validated.
  ...
  -- Caminho normal: contrato validated → explode checklist
  UPDATE public.qa_processos
     SET pagamento_status = 'confirmado',
         status = CASE
...
-- Reconciliar processos do Willian (cliente_id=189): pagamentos confirmados +
-- contratos pendentes ⇒ devem estar em aguardando_assinatura.
   ...
   AND EXISTS (
     SELECT 1 FROM public.qa_vendas v
      WHERE v.id = p.venda_id
        AND v.status = 'PAGO'
        AND v.cobranca_status = 'confirmada'
   )
   AND NOT EXISTS (
     SELECT 1 FROM public.qa_contracts c
      WHERE c.venda_id = p.venda_id AND c.status = 'validated'
   );

INSERT INTO public.qa_processo_eventos (processo_id, tipo_evento, descricao, ator)
SELECT p.id,
       'aguardando_assinatura_contrato',
       'Reconciliação: pagamento confirmado, aguardando assinatura do contrato. Checklist permanece bloqueado.',
       'sistema'
  FROM public.qa_processos p
 WHERE p.cliente_id = 189 AND p.status = 'aguardando_assinatura';