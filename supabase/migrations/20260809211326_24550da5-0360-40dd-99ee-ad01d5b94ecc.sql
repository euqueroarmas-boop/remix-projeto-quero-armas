...
AS $$
  ...
  prox AS (
    SELECT DISTINCT ON (p.processo_id) p.processo_id, p.nome, p.prio
    FROM pend p
  ...
  SELECT
    ...
    CASE
      ...
      WHEN b.proximo_prio = 1 THEN 'EFETIVA NECESSIDADE'
      WHEN b.proximo_prio = 2 THEN 'EXAMES'
      WHEN b.proximo_prio = 3 THEN 'CERTIDOES'
      WHEN b.proximo_prio = 9 THEN 'PROTOCOLO'
      ELSE 'DOCUMENTOS'
    END AS fase,
    b.total_docs,
