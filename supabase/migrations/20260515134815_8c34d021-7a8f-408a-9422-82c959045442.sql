...
BEGIN
  ...
  VALUES (NEW.id, 'contrato_assinado_apto_para_liberacao',
          jsonb_build_object('fase','2C-7.2','contract_venda_id', NEW.venda_id));

  SELECT id, id_legado, status, cobranca_status, cliente_id INTO v_venda
    FROM public.qa_vendas WHERE id_legado = NEW.venda_id LIMIT 1;
  IF NOT FOUND THEN
    SELECT id, id_legado, status, cobranca_status, cliente_id INTO v_venda
      FROM public.qa_vendas WHERE id = NEW.venda_id LIMIT 1;
  END IF;
  IF NOT FOUND THEN
    INSERT INTO public.qa_contract_events (contract_id, event_type, event_payload)
  ...
  IF upper(btrim(v_venda.status)) <> 'PAGO' OR v_venda.cobranca_status IS DISTINCT FROM 'confirmada' THEN
    INSERT INTO public.qa_contract_events (contract_id, event_type, event_payload)
    VALUES (NEW.id, 'liberacao_recusada_pagamento_invalido',
            jsonb_build_object('venda_id_real', v_venda.id, 'venda_id_legado', v_venda.id_legado,
  ...
  BEGIN
    SELECT decrypted_secret INTO v_internal_token
      FROM vault.decrypted_secrets WHERE name='QA_CONTRACT_RELEASE_TOKEN' LIMIT 1;
  ...
  IF v_internal_token IS NULL OR length(v_internal_token) < 16 THEN
    ...
    VALUES (NEW.id, 'liberacao_falhou',
            jsonb_build_object('motivo','qa_contract_release_token_ausente_no_vault',
                               'vault_error', v_vault_err,
                               'token_len', COALESCE(length(v_internal_token),0),
                               'venda_id_real', v_venda.id, 'venda_id_legado', v_venda.id_legado));
    RETURN NEW;
  END IF;
