-- Trigger verde: exigência cumprida.
-- Dispara qa-notify-event com evento 'exigencia_cumprida' sempre que um slot
-- de processo (qa_processo_documentos) passa para status='aprovado'.
-- Este é o momento canônico: o trigger existente qa_doc_hub_satisfaz_exigencias_trigger
-- (20260618030000) já propaga da aprovação no hub (qa_documentos_cliente) para
-- o slot. Escutamos aqui, no destino, para não pegar aprovações do hub que
-- não cumprem exigência de processo.
--
-- Reusa o motor qa-notify-event já existente. Nenhum motor novo.
...
BEGIN
  -- Só quando slot passa PARA aprovado
