-- 1) Coluna opcional para vincular o processo à solicitação que originou a venda
ALTER TABLE public.qa_processos
  ADD COLUMN IF NOT EXISTS solicitacao_id uuid NULL
    REFERENCES public.qa_solicitacoes_servico(id) ON DELETE SET NULL;

CREATE INDEX IF NOT EXISTS idx_qa_processos_solicitacao_id
  ON public.qa_processos(solicitacao_id)
  WHERE solicitacao_id IS NOT NULL;

-- 2) RPC atualizada: propaga solicitacao_id da venda → processo e atualiza a solicitação.
--    Mantém TODO o comportamento anterior; apenas adiciona o vínculo quando ele existir.
  ...
  INSERT INTO public.qa_processo_eventos (
    ...
    'Processo criado a partir da venda #' || p_venda_id::text ||
      ...
      CASE WHEN v_solicitacao_id IS NOT NULL
        THEN ' (solicitação ' || v_solicitacao_id::text || ')'
        ELSE '' END,
    'equipe_operacional'
  );
