-- Fix 1: catálogo do CR (serviço 44) — slug genérico 'documento' → 'rg_com_cpf'
--
-- O registro 5129db56-… tinha tipo_documento='documento' (slug genérico),
-- impedindo o matching com o CIN do Hub (tipo_documento='cin'), pois a tabela
-- qa_tipo_documento_aliases só mapeia 'rg_com_cpf' ↔ 'cin', não 'documento'.
-- Todos os outros serviços já usam 'rg_com_cpf' como slug canônico.

UPDATE public.qa_servicos_documentos
   SET tipo_documento = 'rg_com_cpf'
 WHERE id = '5129db56-8295-4ad6-8886-5b8fa5582f23'
   AND tipo_documento = 'documento';   -- guarda idempotente

-- Fix 2: qa_explodir_checklist_processo — adiciona alias lookup no cofre
--
-- O CTE cofre_validos fazia match por igualdade exata:
--   pd.tipo_documento = cv.tipo_documento
-- ignorando qa_tipo_documento_aliases, que já era usado corretamente em:
--   qa_doc_hub_satisfaz_exigencias_processo (trigger de upload)
--   qa_processo_rever_exigencias (backfill retroativo)
-- Resultado: CIN do Hub (hub_tipo='cin') não reaproveitar slot 'rg_com_cpf'
-- mesmo existindo o alias ('rg_com_cpf', 'cin') na tabela.
--
-- Correção: espelha exatamente o padrão de qa_processo_rever_exigencias.
...
BEGIN
  ...
  -- Espelha qa_processo_rever_exigencias: um doc hub do tipo 'cin' satisfaz
  -- o slot 'rg_com_cpf' do checklist via qa_tipo_documento_aliases.
  ...
  -- SLICE 2.1: 5 slots de endereço + auto-aproveitamento
  -- Inclui Autorização de Compra Atirador (50) e Caçador (51)
