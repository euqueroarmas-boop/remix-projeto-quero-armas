-- Estende a lógica de "5 anos de comprovante de endereço" para os processos
-- de Autorização de Compra como Caçador (51) e Atirador Esportivo (50),
-- igualando ao comportamento já existente para Concessão de CR (44) e Posse (31).

-- ─── 1. qa_seed_endereco_5_anos ──────────────────────────────────────────────
...
BEGIN
  ...
  -- Agora inclui Autorização de Compra Atirador (50) e Caçador (51)
  IF v_proc.servico_id NOT IN (31, 44, 50, 51) THEN RETURN 0; END IF;
...
-- ─── 2. qa_explodir_checklist_processo ───────────────────────────────────────
...
-- ─── 3. Backfill: adiciona slots de endereço nos processos 50/51 existentes ──

DO $$
DECLARE r record;
BEGIN
  FOR r IN SELECT id FROM public.qa_processos WHERE servico_id IN (50, 51) LOOP
    BEGIN
