...
CREATE OR REPLACE FUNCTION public.qa_painel_progresso_clientes()
 RETURNS TABLE(processo_id uuid, cliente_id integer, cliente_nome text, cliente_email text, servico_nome text, status text, fase text, total_docs integer, entregues integer, dispensados integer, reaproveitados integer, documentos_pendentes integer, perguntas_pendentes integer, em_analise integer, grupo_atual text, grupo_total integer, grupo_concluidos integer, grupos_total integer, grupo_indice integer, grupos_concluidos integer, grupos_restantes integer, bloqueado_por_prerequisito boolean, proximo_doc text, proximo_tipo text, ultima_atividade timestamp with time zone, dias_parado integer, cobrancas integer, criado_em timestamp with time zone)
 LANGUAGE sql
