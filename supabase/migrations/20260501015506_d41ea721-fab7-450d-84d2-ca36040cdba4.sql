-- 1) Estende constraint de status para incluir 'pre_validado'
DO $$
DECLARE r record;
BEGIN
  FOR r IN
...
ALTER TABLE public.qa_processo_documentos
  ADD CONSTRAINT chk_qa_processo_documentos_status CHECK (status = ANY (ARRAY[
    ...
    'pendente_reenvio','pre_validado'
  ]));

-- 2) DROP + CREATE (mudança de tipo de retorno)
DROP FUNCTION IF EXISTS public.qa_explodir_checklist_processo(uuid);

CREATE FUNCTION public.qa_explodir_checklist_processo(p_processo_id uuid)
...
BEGIN
  ...
  -- (c) PRÉ-PREENCHER textuais
  IF v_cli.end1_cep IS NOT NULL AND v_cli.end1_logradouro IS NOT NULL
     AND v_cli.end1_cidade IS NOT NULL AND v_cli.end1_estado IS NOT NULL THEN
    UPDATE public.qa_processo_documentos
       SET observacoes = COALESCE(observacoes,'') ||
             ...
             v_cli.end1_logradouro || ', ' || COALESCE(v_cli.end1_numero,'s/n') ||
             ' — ' || v_cli.end1_cidade || '/' || v_cli.end1_estado ||
             ' — CEP ' || v_cli.end1_cep,
           updated_at = now()
     WHERE processo_id = p_processo_id
