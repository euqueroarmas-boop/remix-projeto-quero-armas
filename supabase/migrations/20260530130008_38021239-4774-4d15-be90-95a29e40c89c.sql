...
BEGIN
  ...
  IF to_regclass('public.qa_solicitacoes_servico') IS NOT NULL THEN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='qa_solicitacoes_servico' AND column_name='cliente_id') THEN
  ...
  IF to_regclass('public.qa_status_eventos') IS NOT NULL THEN
    IF cardinality(v_solicitacao_ids) > 0 THEN
      EXECUTE 'DELETE FROM public.qa_status_eventos WHERE cliente_id = $1 OR solicitacao_id = ANY($2)'
      USING p_cliente_id, v_solicitacao_ids;
    ELSE
      EXECUTE 'DELETE FROM public.qa_status_eventos WHERE cliente_id = $1'
      USING p_cliente_id;
    END IF;
  END IF;

  IF to_regclass('public.qa_solicitacao_eventos') IS NOT NULL THEN
    EXECUTE 'DELETE FROM public.qa_solicitacao_eventos WHERE cliente_id = $1'
    USING p_cliente_id;
  END IF;

  IF to_regclass('public.qa_solicitacoes_servico') IS NOT NULL THEN
    EXECUTE 'DELETE FROM public.qa_solicitacoes_servico WHERE cliente_id = $1'
    USING p_cliente_id;
  END IF;
  ...
  IF to_regclass('public.qa_cliente_armas') IS NOT NULL THEN
    EXECUTE 'DELETE FROM public.qa_cliente_armas WHERE qa_cliente_id = $1'
    USING p_cliente_id;
  END IF;
  ...
  IF to_regclass('public.qa_municoes_saldos') IS NOT NULL THEN
    EXECUTE 'DELETE FROM public.qa_municoes_saldos WHERE cliente_id = $1'
    USING p_cliente_id;
  END IF;
  IF to_regclass('public.qa_municoes_em_revisao') IS NOT NULL THEN
    EXECUTE 'DELETE FROM public.qa_municoes_em_revisao WHERE cliente_id = $1'
    USING p_cliente_id;
  END IF;
  ...
  IF to_regclass('public.qa_exames_cliente_status') IS NOT NULL THEN
    EXECUTE 'DELETE FROM public.qa_exames_cliente_status WHERE cliente_id = $1'
    USING p_cliente_id;
  END IF;
  ...
  IF to_regclass('public.qa_arsenal_fila_revisao') IS NOT NULL THEN
    EXECUTE 'DELETE FROM public.qa_arsenal_fila_revisao WHERE cliente_id = $1'
    USING p_cliente_id;
  END IF;
  ...
  IF to_regclass('public.qa_cliente_credenciais_audit') IS NOT NULL THEN
    EXECUTE 'DELETE FROM public.qa_cliente_credenciais_audit WHERE cliente_id = $1'
    USING p_cliente_id;
  END IF;
  ...
  DELETE FROM public.qa_vendas WHERE cliente_id = v_id_legado OR cliente_id = p_cliente_id OR id = ANY(v_venda_ids) OR id = ANY(v_venda_legado_ids);
