-- ============================================================================
-- AUDITORIA COMPLETA — cliente 59 (IGOR ANTONINO FERREIRA DA SILVA)
-- CPF 459.305.848-18 · igorb26@outlook.com
-- ----------------------------------------------------------------------------
-- Um único SELECT, somente leitura. Cada linha sai com o nome do bloco
-- ("01_cliente", "16_checklist_processo", …) e o registro inteiro em JSON.
-- Rodar tudo de uma vez no SQL Editor e devolver o resultado COMPLETO
-- (se a tela cortar, usar Export → JSON/CSV).
-- Campos de senha (senha_temporaria, senha_gov_*) ficam de fora do resultado.
-- ============================================================================

SELECT '01_cliente' AS bloco,
       to_jsonb(c) - 'senha_temporaria' - 'senha_temporaria_expira_em' AS dado
...
SELECT '02_cadastro_publico', to_jsonb(cp) - 'consentimento_texto'
 ...
 WHERE cp.cliente_id_vinculado = 59
...
SELECT '08_itens_venda',
  ...
  JOIN public.qa_vendas v ON v.id = iv.venda_id AND v.cliente_id = 59
...
SELECT * FROM (
  SELECT '09_venda_eventos', to_jsonb(ve)
   ...
   WHERE ve.venda_id IN (SELECT id FROM public.qa_vendas WHERE cliente_id = 59)
      OR ve.cliente_id = 59 OR ve.qa_cliente_id = 59
...
SELECT '10_contratos',
 ...
 WHERE ct.cliente_id = 59

UNION ALL
SELECT '11_contrato_assinaturas', to_jsonb(cs) - 'validation_details'
  FROM public.qa_contract_signatures cs
 WHERE cs.contract_id IN (SELECT id FROM public.qa_contracts WHERE cliente_id = 59)

UNION ALL
SELECT * FROM (
  SELECT '12_contrato_eventos', to_jsonb(ce)
    FROM public.qa_contract_events ce
   WHERE ce.contract_id IN (SELECT id FROM public.qa_contracts WHERE cliente_id = 59)
...
SELECT '13_solicitacoes_servico', to_jsonb(ss)
 ...
 WHERE ss.cliente_id = 59

UNION ALL
SELECT * FROM (
  SELECT '14_solicitacao_eventos', to_jsonb(se)
    FROM public.qa_solicitacao_eventos se
   WHERE se.cliente_id = 59
      OR se.solicitacao_id IN (SELECT id FROM public.qa_solicitacoes_servico WHERE cliente_id = 59)
   ORDER BY se.created_at DESC LIMIT 500) s14

UNION ALL
SELECT '15_processos', to_jsonb(p)
  FROM public.qa_processos p
 WHERE p.cliente_id = 59
...
SELECT '16_checklist_processo',
 ...
 WHERE pd.cliente_id = 59
    OR pd.processo_id IN (SELECT id FROM public.qa_processos WHERE cliente_id = 59)
...
SELECT * FROM (
  SELECT '20_processo_eventos', to_jsonb(pe)
   ...
   WHERE pe.processo_id IN (SELECT id FROM public.qa_processos WHERE cliente_id = 59)
...
SELECT * FROM (
  SELECT '21_status_eventos', to_jsonb(st)
   ...
   WHERE st.cliente_id = 59
      OR st.processo_id::text IN (SELECT id::text FROM public.qa_processos WHERE cliente_id = 59)
...
SELECT * FROM (
  SELECT '37_login_eventos', to_jsonb(le)
   ...
   WHERE le.qa_cliente_id::text = '59'
...
SELECT '41_validades_resumo',
  ...
  FROM (
    SELECT 'HUB' AS fonte,
      ...
      FROM public.qa_documentos_cliente d
     WHERE d.qa_cliente_id = 59 AND d.data_validade IS NOT NULL AND d.status <> 'excluido'
    UNION ALL
    ...
    SELECT 'CR', coalesce(cr.numero_cr, 'CR'), 'registrado', cr.validade_cr::date
      FROM public.qa_cadastro_cr cr
     WHERE cr.cliente_id = 59 AND cr.validade_cr IS NOT NULL
    UNION ALL
    SELECT 'CRAF', coalesce(cf.nome_craf, 'CRAF'), 'registrado', cf.data_validade::date
      FROM public.qa_crafs cf
     WHERE cf.cliente_id = 59 AND cf.data_validade IS NOT NULL
    UNION ALL
    SELECT 'GTE', coalesce(g.nome_gte, 'GTE'), 'registrado', g.data_validade::date
      FROM public.qa_gtes g
     WHERE g.cliente_id = 59 AND g.data_validade IS NOT NULL
  ) vr
