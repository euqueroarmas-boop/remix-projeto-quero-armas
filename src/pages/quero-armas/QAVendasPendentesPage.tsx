...
function badgeForStatus(status: StatusValidacao | null, hasProc: boolean) {
  ...
  if (status === "aprovado") return { txt: "Aprovado", cls: "bg-[#F7F7F8] text-[#2F3337] border-[#D1D3D6]" };
          ...
          {eventos.length === 0 ? (
            ...
            <div key={e.id} className="border border-slate-200 rounded-md p-2 bg-slate-50">
              <div className="flex items-center justify-between gap-2">
                <span className="text-[10px] font-bold uppercase tracking-wider text-[#2F3337]">{e.tipo_evento}</span>
  ...
  return (
    <Dialog open={open} onOpenChange={(o) => !submitting && !o && onClose()}>
      <DialogContent className="max-w-md bg-white max-h-[85vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="text-slate-800 text-sm font-bold uppercase tracking-wider flex items-center gap-2">
            <CreditCard className="h-4 w-4 text-[#2F3337]" /> Gerar cobrança
          </DialogTitle>
        </DialogHeader>
        <div className="space-y-3 mt-2 text-[12px]">
          <p className="text-slate-700">
          ...
          <div className="grid grid-cols-2 gap-2 pt-2">
            <Button type="button" variant="ghost" onClick={onClose} disabled={submitting}
              className="h-10 text-xs rounded-md text-slate-600 hover:text-slate-800">Cancelar</Button>
            <Button type="button" onClick={handle} disabled={submitting}
              className="h-10 text-xs rounded-md bg-[#2F3337] hover:bg-[#24272A] text-white shadow-sm disabled:opacity-60">
  ...
  const carregar = useCallback(async () => {
    ...
    try {
      ...
      const { data: vRes, error: vErr } = await supabase
        ...
        .select("id, id_legado, cliente_id, status, status_validacao_valor, valor_a_pagar, valor_informado_cliente, valor_aprovado, origem_proposta, motivo_correcao, data_cadastro, forma_pagamento, asaas_payment_id, cobranca_status, asaas_invoice_url")
          ...
          return (
            <button key={f.id} onClick={() => setFiltro(f.id)}
              className={`h-8 px-3 inline-flex items-center gap-1.5 rounded-full text-[11px] uppercase tracking-wider font-bold border transition ${
                active ? "bg-[#2F3337] text-white border-[#2F3337]" : "bg-white text-slate-600 border-slate-200 hover:bg-slate-50"
              }`}>
              {f.label}
                ...
                <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-3">
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="mt-2 grid sm:grid-cols-2 gap-x-4 gap-y-0.5 text-[12px] text-slate-700">
                      ...
                      {cli?.email && (
                        <div className="flex items-center gap-1.5 truncate">
                          ...
                          <a href={`mailto:${cli.email}`} className="text-[#2F3337] hover:underline truncate">{cli.email}</a>
                        </div>
                      )}
                      {cli?.celular && (
                        <div className="flex items-center gap-1.5">
                    ...
                    <div className="mt-2 flex flex-wrap items-center gap-1.5 text-[10px]">
                      ...
                      {v.asaas_payment_id ? (
                        <>
                          <span className="px-1.5 py-0.5 rounded border border-[#D1D3D6] bg-[#F7F7F8] text-[#2F3337] font-mono">
                            {v.asaas_payment_id}
                          </span>
                          ...
                          {(v as any).asaas_invoice_url && (
                            <a href={(v as any).asaas_invoice_url} target="_blank" rel="noopener noreferrer"
                              className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded border border-slate-200 bg-white text-slate-600 hover:text-[#2F3337] hover:border-[#D1D3D6]">
                  ...
                  <div className="flex md:flex-col items-stretch gap-1.5 md:min-w-[180px]">
                    ...
                    {status === "aprovado"
                      ...
                      <Button onClick={() => setCobrancaVenda(v)}
                        className="h-8 text-[10px] rounded-md bg-[#2F3337] hover:bg-[#24272A] text-white shadow-sm">
