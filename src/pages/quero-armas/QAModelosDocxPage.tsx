    ...
    <div className="space-y-6 max-w-4xl">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-neutral-500 mt-1">Templates para geração de peças editáveis</p>
        </div>
      ...
      {/* Variables info */}
      <div className="bg-[#111111] border border-neutral-800/40 rounded-xl p-4">
        <h3 className="text-xs font-medium text-neutral-400 mb-2">Variáveis Suportadas</h3>
        <div className="flex flex-wrap gap-2">
          {VARIAVEIS.map(v => (
            <code key={v} className="text-[10px] px-2 py-1 rounded bg-[#0a0a0a] text-amber-400/70 border border-neutral-800/40 font-mono">{v}</code>
          ))}
        </div>
      </div>
      ...
      {showForm && (
        <div className="bg-[#111111] border border-amber-500/20 rounded-xl p-5 space-y-4">
          <h3 className="text-sm font-medium text-neutral-300">Cadastrar Novo Modelo</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-2">
              <Label className="text-neutral-400 text-xs">Nome do Modelo</Label>
              <Input value={nomeModelo} onChange={e => setNomeModelo(e.target.value)}
                className="bg-[#0a0a0a] border-neutral-700 text-neutral-100" placeholder="Ex: Petição Inicial - Registro de Arma" />
            </div>
            <div className="space-y-2">
              <Label className="text-neutral-400 text-xs">Tipo de Peça</Label>
              <Select value={tipoPeca} onValueChange={setTipoPeca}>
                <SelectTrigger className="bg-[#0a0a0a] border-neutral-700 text-neutral-300"><SelectValue /></SelectTrigger>
                <SelectContent>
          ...
          <div className="space-y-2">
            ...
            <Textarea value={descricao} onChange={e => setDescricao(e.target.value)}
              className="bg-[#0a0a0a] border-neutral-700 text-neutral-100" rows={2} placeholder="Descrição do modelo e quando utilizá-lo..." />
          </div>
          <div className="space-y-2">
            <Label className="text-neutral-400 text-xs">Arquivo Template (.docx)</Label>
            <Input type="file" accept=".docx" onChange={e => setSelectedFile(e.target.files?.[0] || null)}
              className="bg-[#0a0a0a] border-neutral-700 text-neutral-400" />
          </div>
          <div className="flex gap-2">
            ...
            <Button variant="ghost" onClick={() => setShowForm(false)} className="text-neutral-400">Cancelar</Button>
          </div>
        </div>
      )}
      ...
      {loading ? (
        ...
        <div className="space-y-2">
          {modelos.map((m: any) => (
            <div key={m.id} className="flex items-center gap-4 bg-[#111111] border border-neutral-800/40 rounded-lg p-4 hover:border-neutral-700 transition-all">
              {m.ativo ? <CheckCircle className="h-4 w-4 text-emerald-400 shrink-0" /> : <XCircle className="h-4 w-4 text-neutral-600 shrink-0" />}
              <div className="flex-1 min-w-0">
                <div className="text-sm font-medium text-neutral-200">{m.nome_modelo}</div>
                <div className="text-xs text-neutral-500 flex items-center gap-2 mt-0.5">
                  <span className="px-1.5 py-0.5 rounded bg-neutral-800 text-neutral-400">{m.tipo_peca?.replace(/_/g, " ")}</span>
                  <span>v{m.versao}</span>
                  <span>{new Date(m.created_at).toLocaleDateString("pt-BR")}</span>
                </div>
                {m.descricao && <p className="text-xs text-neutral-600 mt-1 line-clamp-1">{m.descricao}</p>}
              </div>
