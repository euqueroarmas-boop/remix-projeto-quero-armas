...
export default function QAHistoricoPage() {
  ...
  const statusBadge = (s: string) => {
    const map: Record<string, { bg: string; text: string }> = {
      ...
      corrigido: { bg: "bg-[#F7F7F8]", text: "text-[#2F3337]" },
        ...
        <div className="flex gap-1 bg-white border rounded-xl p-1" style={{ borderColor: "hsl(220 13% 91%)" }}>
          <button onClick={() => setTab("geracoes")}
            className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${
              tab === "geracoes" ? "bg-[#2F3337] text-white shadow-sm" : "text-slate-500 hover:text-slate-700 hover:bg-slate-50"
            }`}>Peças</button>
          <button onClick={() => setTab("consultas")}
            className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${
              tab === "consultas" ? "bg-[#2F3337] text-white shadow-sm" : "text-slate-500 hover:text-slate-700 hover:bg-slate-50"
            }`}>Consultas</button>
        </div>
      </div>
                  ...
                  <div className="flex items-center gap-1 shrink-0">
                    ...
                    {tab === "geracoes" && item.minuta_gerada && (
                      <button
                        ...
                        className="h-8 w-8 rounded-lg flex items-center justify-center hover:bg-[#F7F7F8] transition-colors"
                ...
                <div className="space-y-0.5 mt-1">
                  {(detailItem?.fontes_recuperadas_json || detailItem?.fundamentos_utilizados_json)?.map((f: any, i: number) => (
                    <div key={i} className="flex items-center gap-1.5 text-xs" style={{ color: "hsl(220 10% 45%)" }}>
                      <span className="w-1.5 h-1.5 rounded-full bg-[#2F3337] shrink-0" />
            ...
            {tab === "geracoes" && detailItem?.minuta_gerada && (
              <div className="flex flex-wrap gap-2 pt-2 border-t border-slate-200">
                <button
                  ...
                  className="h-9 px-4 rounded-lg text-xs font-semibold flex items-center gap-1.5 bg-[#F7F7F8] text-[#2F3337] border border-[#D1D3D6] hover:bg-[#F7F7F8] transition-all"
                >
