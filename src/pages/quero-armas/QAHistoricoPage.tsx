...
export default function QAHistoricoPage() {
  ...
  const statusBadge = (s: string) => {
    const map: Record<string, string> = {
      rascunho: "text-neutral-500", em_revisao: "text-amber-400", corrigido: "text-blue-400",
      aprovado: "text-emerald-400", aprovado_como_referencia: "text-purple-400", rejeitado: "text-red-400",
    };
    return map[s] || "text-neutral-500";
  };

  return (
    <div className="space-y-3 md:space-y-4 max-w-5xl">
      <div className="flex items-center justify-between gap-2">
        <h1 className="text-sm md:text-base font-semibold text-neutral-300">Histórico</h1>
        <div className="flex gap-0.5">
          <button onClick={() => setTab("geracoes")}
            className={`px-2.5 py-1 rounded text-[10px] md:text-[11px] uppercase tracking-wider font-medium transition-colors ${
              tab === "geracoes" ? "bg-[#7a1528] text-neutral-300" : "text-neutral-600 hover:text-neutral-400"
            }`}>Peças</button>
          <button onClick={() => setTab("consultas")}
            className={`px-2.5 py-1 rounded text-[10px] md:text-[11px] uppercase tracking-wider font-medium transition-colors ${
              tab === "consultas" ? "bg-[#7a1528] text-neutral-300" : "text-neutral-600 hover:text-neutral-400"
            }`}>Consultas</button>
        </div>
      </div>

      {loading ? (
        <div className="flex justify-center py-10"><div className="w-4 h-4 border-2 border-neutral-700 border-t-slate-400 rounded-full animate-spin" /></div>
      ) : items.length === 0 ? (
        <div className="text-center py-10 text-neutral-600 text-[11px]">Nenhum registro</div>
      ) : (
        <div className="bg-[#111111] border border-[#1c1c1c] rounded overflow-hidden">
          {/* Desktop header */}
          <div className="hidden md:grid grid-cols-[1fr_120px_80px_80px_60px] gap-2 px-3 py-1.5 border-b border-[#1c1c1c] text-[9px] text-neutral-600 uppercase tracking-[0.12em]">
            <span>Título</span><span>Tipo</span><span>Status</span><span>Data</span><span className="text-right">Ações</span>
          </div>
          {items.map((item: any) => (
            <div key={item.id} className="border-b border-[#1c1c1c]/40 hover:bg-[#7a1528]/50 transition-colors">
              {/* Mobile */}
              <div className="md:hidden px-2.5 py-1.5">
                <div className="flex items-center justify-between gap-2">
                  <span className="text-[11px] text-neutral-400 truncate flex-1 min-w-0">{item.titulo_geracao || item.caso_titulo || "Sem título"}</span>
                  <span className={`text-[8px] font-mono shrink-0 ${statusBadge(item.status_revisao || "")}`}>
                    {(item.status_revisao || "—").replace(/_/g, " ")}
                  </span>
                </div>
                <div className="flex items-center gap-2 mt-0.5">
                  <span className="text-[9px] text-neutral-600 font-mono truncate">{(item.tipo_peca || "—").replace(/_/g, " ")}</span>
                  <span className="text-[8px] text-neutral-700 font-mono tabular-nums ml-auto">{new Date(item.created_at).toLocaleDateString("pt-BR")}</span>
                  <button onClick={() => setDetailItem(item)} className="p-0.5 text-neutral-600 hover:text-neutral-400"><Eye className="h-3 w-3" /></button>
                  {tab === "geracoes" && canReview && (
                    <button onClick={() => openReview(item)} className="p-0.5 text-neutral-600 hover:text-neutral-400"><MessageSquare className="h-3 w-3" /></button>
                  )}
                </div>
              </div>
              {/* Desktop */}
              <div className="hidden md:grid grid-cols-[1fr_120px_80px_80px_60px] gap-2 px-3 py-2 items-center">
                <span className="text-[12px] text-neutral-400 truncate">{item.titulo_geracao || item.caso_titulo || "Sem título"}</span>
                <span className="text-[10px] text-neutral-600 font-mono truncate">{(item.tipo_peca || "—").replace(/_/g, " ")}</span>
                <span className={`text-[10px] font-mono ${statusBadge(item.status_revisao || "")}`}>{(item.status_revisao || "—").replace(/_/g, " ")}</span>
                <span className="text-[10px] text-neutral-700 font-mono tabular-nums">{new Date(item.created_at).toLocaleDateString("pt-BR")}</span>
                <div className="flex justify-end gap-0.5">
                  <button onClick={() => setDetailItem(item)} className="p-1 text-neutral-600 hover:text-neutral-400"><Eye className="h-3.5 w-3.5" /></button>
                  {tab === "geracoes" && canReview && (
                    <button onClick={() => openReview(item)} className="p-1 text-neutral-600 hover:text-neutral-400"><MessageSquare className="h-3.5 w-3.5" /></button>
                  )}
                </div>
              </div>
            </div>
      ...
      <Dialog open={!!detailItem} onOpenChange={() => setDetailItem(null)}>
        <DialogContent className="bg-[#0a0a0a] border-[#1c1c1c] text-neutral-300 max-w-3xl max-h-[90vh] overflow-y-auto p-3 md:p-6">
          <DialogHeader>
            <DialogTitle className="text-neutral-300 text-sm">{detailItem?.titulo_geracao || detailItem?.caso_titulo || "Detalhes"}</DialogTitle>
          </DialogHeader>
          <div className="space-y-3 mt-1">
            {(detailItem?.fontes_recuperadas_json || detailItem?.fundamentos_utilizados_json)?.length > 0 && (
              <div>
                <span className="text-[9px] text-neutral-600 uppercase tracking-[0.12em]">Fontes</span>
            ...
            <div>
              <span className="text-[9px] text-neutral-600 uppercase tracking-[0.12em]">{tab === "geracoes" ? "Minuta" : "Resposta"}</span>
              <div className="text-[11px] text-neutral-400 whitespace-pre-wrap leading-relaxed bg-[#0a0a0a] rounded p-2.5 max-h-[350px] overflow-y-auto mt-1 font-serif">
      ...
      <Dialog open={reviewOpen} onOpenChange={setReviewOpen}>
        <DialogContent className="bg-[#0a0a0a] border-[#1c1c1c] text-neutral-300 max-w-4xl max-h-[90vh] overflow-y-auto p-3 md:p-6">
          <DialogHeader><DialogTitle className="text-neutral-300 text-sm">Revisão</DialogTitle></DialogHeader>
          <div className="space-y-3 mt-1">
            <div className="space-y-1">
              <Label className="text-[9px] text-neutral-600 uppercase tracking-wider">Minuta (editável)</Label>
              <Textarea value={reviewText} onChange={e => setReviewText(e.target.value)}
                className="bg-[#0a0a0a] border-[#1c1c1c] text-neutral-400 min-h-[200px] md:min-h-[300px] font-serif text-[11px]" />
            </div>
            <div className="space-y-1">
              <Label className="text-[9px] text-neutral-600 uppercase tracking-wider">Justificativa</Label>
              <Textarea value={reviewJustificativa} onChange={e => setReviewJustificativa(e.target.value)}
                className="bg-[#0a0a0a] border-[#1c1c1c] text-neutral-400 min-h-[50px] text-[11px]" placeholder="Opcional" />
            </div>
