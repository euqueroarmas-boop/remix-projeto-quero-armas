  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-2xl font-bold text-neutral-100">Legislação</h1>
          <p className="text-sm text-neutral-500 mt-1">Base normativa controlada</p>
        </div>
        {canEdit && (
          <Dialog open={open} onOpenChange={setOpen}>
            <DialogTrigger asChild>
              <Button className="bg-amber-600 hover:bg-amber-700"><Plus className="h-4 w-4 mr-2" /> Nova Norma</Button>
            </DialogTrigger>
            <DialogContent className="bg-[#111111] border-neutral-800 text-neutral-100 max-w-2xl max-h-[90vh] overflow-y-auto">
              <DialogHeader><DialogTitle className="text-neutral-100">Cadastrar Norma</DialogTitle></DialogHeader>
                ...
                <div className="grid grid-cols-3 gap-4">
                  <div className="space-y-2">
                    ...
                    <Input value={form.numero_norma} onChange={e => setForm(f => ({ ...f, numero_norma: e.target.value }))}
                      className="bg-[#0a0a0a] border-neutral-700 text-neutral-100" placeholder="10.826" />
                  </div>
                  <div className="space-y-2">
                    <Label className="text-neutral-300">Ano</Label>
                    <Input type="number" value={form.ano_norma} onChange={e => setForm(f => ({ ...f, ano_norma: e.target.value }))}
                      className="bg-[#0a0a0a] border-neutral-700 text-neutral-100" placeholder="2003" />
                  </div>
                  <div className="space-y-2">
                    <Label className="text-neutral-300">Órgão Emissor</Label>
                    <Input value={form.orgao_emissor} onChange={e => setForm(f => ({ ...f, orgao_emissor: e.target.value }))}
                      className="bg-[#0a0a0a] border-neutral-700 text-neutral-100" />
                  </div>
                </div>
                <div className="space-y-2">
                  <Label className="text-neutral-300">Ementa</Label>
                  <Textarea value={form.ementa} onChange={e => setForm(f => ({ ...f, ementa: e.target.value }))}
                    className="bg-[#0a0a0a] border-neutral-700 text-neutral-100 min-h-[80px]" />
                </div>
                <div className="space-y-2">
                  <Label className="text-neutral-300">Texto Integral</Label>
                  <Textarea value={form.texto_integral} onChange={e => setForm(f => ({ ...f, texto_integral: e.target.value }))}
                    className="bg-[#0a0a0a] border-neutral-700 text-neutral-100 min-h-[150px]" />
                </div>
                <div className="space-y-2">
                  <Label className="text-neutral-300">Palavras-chave (separadas por vírgula)</Label>
                  <Input value={form.palavras_chave} onChange={e => setForm(f => ({ ...f, palavras_chave: e.target.value }))}
                    className="bg-[#0a0a0a] border-neutral-700 text-neutral-100" placeholder="armas, posse, CAC" />
                </div>
      ...
      {loading ? (
        ...
        <div className="space-y-2">
          {normas.map((n: any) => (
            <div key={n.id} className="bg-[#111111] border border-neutral-800/40 rounded-lg p-4 hover:border-neutral-700 transition-all">
              <div className="flex items-start justify-between gap-3">
                <div className="flex-1 min-w-0">
                  <div className="text-sm font-medium text-neutral-200">{n.titulo_norma}</div>
                  <div className="text-xs text-neutral-500 flex items-center gap-2 mt-1">
                    <span className="px-1.5 py-0.5 rounded bg-neutral-800 text-neutral-400">{n.tipo_norma?.replace(/_/g, " ")}</span>
                    {n.numero_norma && <span>Nº {n.numero_norma}</span>}
                    {n.ano_norma && <span>({n.ano_norma})</span>}
                    {n.orgao_emissor && <span>• {n.orgao_emissor}</span>}
                  </div>
                  {n.ementa && <p className="text-xs text-neutral-400 mt-2 line-clamp-2">{n.ementa}</p>}
                </div>
