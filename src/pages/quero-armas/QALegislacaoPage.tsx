  ...
  return (
    <div className="space-y-6">
              ...
              <div className="space-y-4 mt-4">
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <Input value={form.titulo_norma} onChange={e => setForm(f => ({ ...f, titulo_norma: e.target.value }))}
                      className="bg-[#0d0d0d] border-slate-700 text-slate-100" />
                  </div>
                  <div className="space-y-2">
                ...
                <div className="grid grid-cols-3 gap-4">
                  <div className="space-y-2">
                    ...
                    <Input value={form.numero_norma} onChange={e => setForm(f => ({ ...f, numero_norma: e.target.value }))}
                      className="bg-[#0d0d0d] border-slate-700 text-slate-100" placeholder="10.826" />
                  </div>
                  <div className="space-y-2">
                    ...
                    <Input type="number" value={form.ano_norma} onChange={e => setForm(f => ({ ...f, ano_norma: e.target.value }))}
                      className="bg-[#0d0d0d] border-slate-700 text-slate-100" placeholder="2003" />
                  </div>
                  <div className="space-y-2">
                ...
                <div className="space-y-2">
                  ...
                  <Textarea value={form.ementa} onChange={e => setForm(f => ({ ...f, ementa: e.target.value }))}
                    className="bg-[#0d0d0d] border-slate-700 text-slate-100 min-h-[80px]" />
                </div>
                <div className="space-y-2">
                ...
                <div className="space-y-2">
                  ...
                  <Input value={form.palavras_chave} onChange={e => setForm(f => ({ ...f, palavras_chave: e.target.value }))}
                    className="bg-[#0d0d0d] border-slate-700 text-slate-100" placeholder="armas, posse, CAC" />
                </div>
      ...
      {loading ? (
        ...
        <div className="space-y-2">
          {normas.map((n: any) => (
            <div key={n.id} className="bg-[#111111] border border-slate-800/40 rounded-lg p-4 hover:border-slate-700 transition-all">
              <div className="flex items-start justify-between gap-3">
                <div className="flex-1 min-w-0">
                  <div className="text-sm font-medium text-slate-200">{n.titulo_norma}</div>
                  <div className="text-xs text-slate-500 flex items-center gap-2 mt-1">
                    <span className="px-1.5 py-0.5 rounded bg-slate-800 text-slate-400">{n.tipo_norma?.replace(/_/g, " ")}</span>
