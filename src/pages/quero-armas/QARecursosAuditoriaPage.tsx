...
const CAT_META: Record<Categoria, { label: string; tone: string; icon: any; desc: string }> = {
  ...
  EM_CURSO:        { label: "Em Curso",             tone: "bg-[#F7F7F8] text-[#2F3337] border-[#D1D3D6]",          icon: Clock,        desc: "Dentro do prazo de 10 dias" },
  ...
  return (
    <div className="p-4 md:p-6 max-w-7xl mx-auto space-y-5">
      <div className="flex items-center justify-between gap-3">
        ...
        <button
          onClick={exportCSV}
          className="inline-flex items-center gap-1.5 px-3 py-2 text-xs font-semibold rounded-lg bg-[#2F3337] text-white hover:bg-[#26292C] transition"
        >
                  ...
                  <tr key={l.itemId} className="border-t border-slate-100 hover:bg-slate-50">
                    ...
                    <td className="px-3 py-2 text-right tabular-nums">
                      {l.diasSemDecisao !== null
                        ? <span className={l.diasSemDecisao > 10 ? "text-rose-600 font-semibold" : "text-[#2F3337]"}>{l.diasSemDecisao}d s/ decisão</span>
  ...
  return (
    <button
      ...
      className={`px-3 py-1.5 rounded-full text-[11px] font-semibold border transition ${
        active ? "bg-[#2F3337] text-white border-[#2F3337]" : "bg-white text-slate-600 border-slate-200 hover:border-slate-400"
      }`}
    >{label}</button>
  );
}
