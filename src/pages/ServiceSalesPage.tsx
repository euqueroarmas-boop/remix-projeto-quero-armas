  ...
  return (
    <SiteShell>
      <section className="container py-10 sm:py-14">
        <Link to={`/servicos/${service.slug}`} className="mb-6 inline-flex items-center gap-2 font-heading text-xs uppercase tracking-[0.2em] text-muted-foreground transition-colors hover:text-accent">
          <ArrowLeft className="size-3.5" /> Voltar
        </Link>
        <p className="font-heading text-xs uppercase tracking-[0.2em] text-accent">Contratação</p>
        <h1 className="mt-2 font-heading text-2xl font-bold uppercase tracking-tight sm:text-3xl">{service.name}</h1>
        ...
        <div className="mt-8 grid gap-6 lg:grid-cols-[1fr_360px]">
          <div className="rounded-sm border border-border bg-surface-elevated p-6">
            <h2 className="font-heading text-sm uppercase tracking-widest text-accent">Detalhes</h2>
          ...
          <aside className="rounded-sm border border-border bg-surface-elevated p-6">
            <p className="font-heading text-xs uppercase tracking-widest text-muted-foreground">Valor unitário</p>
            <p className="mt-1 font-heading text-3xl font-bold text-accent">{formatBRL(service.base_price_cents)}</p>
