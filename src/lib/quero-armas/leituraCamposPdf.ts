...
export function pareceNomePessoa(v: string | undefined): boolean {
  ...
  // ...e também não TERMINA em preposição/artigo/verbo de ligação. Recorte de
  // frase institucional termina assim ("...SERA DE RESPONSABILIDADE DO",
  // "...PARA A CONSULTA") — nenhum nome brasileiro termina em DO/DE/PARA/E.
  if (/\b(DO|DA|DE|DOS|DAS|E|O|A|EM|AO|NO|NA|PARA|POR|COM|QUE|SER|SERA)$/.test(s)) return false;
