...
const TONE_CLASSES: Record<StatusMeta["tone"], { bg: string; border: string; text: string; ring: string; dot: string }> = {
  ...
  blue:    { bg: "bg-[#F7F7F8]",    border: "border-[#D1D3D6]",    text: "text-[#2F3337]",    ring: "ring-[#2F3337]",    dot: "bg-[#2F3337]" },
  violet:  { bg: "bg-[#F7F7F8]",  border: "border-[#D1D3D6]",  text: "text-[#2F3337]",  ring: "ring-[#2F3337]",  dot: "bg-[#2F3337]" },
  indigo:  { bg: "bg-[#F7F7F8]",  border: "border-[#D1D3D6]",  text: "text-[#2F3337]",  ring: "ring-[#2F3337]",  dot: "bg-[#2F3337]" },
      ...
      <div className="flex items-start justify-between gap-3">
        ...
        <div className="hidden md:flex items-center gap-3 text-[11px] shrink-0">
          <span className="inline-flex items-center gap-1.5 text-slate-500">
            <span className="w-1.5 h-1.5 rounded-full bg-[#2F3337]" /> Ativos: <b className="text-slate-700">{counts.ativos}</b>
          </span>
          <span className="inline-flex items-center gap-1.5 text-slate-500">
          ...
          <div className="flex flex-col md:flex-row md:items-center gap-2 md:gap-3">
            ...
            <div className="relative">
              ...
              <input
                ...
                className="pl-7 pr-3 h-8 w-full md:w-56 text-xs rounded-md border border-slate-200 bg-white focus:outline-none focus:ring-2 focus:ring-[#2F3337]"
              />
            </div>
            <select
              value={sortBy}
              onChange={(e) => setSortBy(e.target.value as SortKey)}
              className="h-8 text-xs rounded-md border border-slate-200 bg-white px-2 focus:outline-none focus:ring-2 focus:ring-[#2F3337]"
            >
          ...
          <>
              ...
              <table className="w-full text-[12.5px]">
                ...
                <tbody>
                  {visible.map((r) => {
                    ...
                    return (
                      <tr key={r.key} className={`border-t border-slate-100 hover:bg-slate-50/60 align-top ${r.isComboGroup ? "bg-[#F7F7F8]" : ""}`}>
                        <td className="px-3 py-2.5 font-medium text-slate-700 align-top">
                          <div className="flex items-center gap-1.5">
                            {r.isComboGroup && <Sparkles className="w-3.5 h-3.5 text-[#2F3337] shrink-0" />}
                            {r.clienteId ? (
                              <Link
                                to={`/clientes?cliente=${r.clienteId}`}
                                className="truncate text-[#2F3337] hover:text-[#2F3337] hover:underline"
                        ...
                        <td className="px-3 py-2.5 text-slate-600 align-top">
                          {r.isComboGroup ? (
                            <div className="space-y-1.5">
                              <span className="inline-flex items-center gap-1 text-[10px] font-bold uppercase tracking-wide text-[#2F3337] bg-[#F7F7F8] border border-[#D1D3D6] rounded px-1.5 py-0.5">
                                COMBO · {r.servicosList.length} serviços
                              </span>
                              <ul className="text-[11.5px] text-slate-700 space-y-0.5">
                                {r.servicosList.map((s, i) => (
                                  <li key={i} className="flex gap-1.5">
                                    <span className="text-[#2F3337]">›</span>
                        ...
                        <td className="px-3 py-2 text-right">
                          {r.clienteId && (
                            <Link
                              to={`/clientes?cliente=${r.clienteId}`}
                              className="inline-flex items-center gap-1 text-[11px] font-medium text-[#2F3337] hover:underline"
                            >
            ...
            <div className="md:hidden divide-y divide-slate-100">
              {visible.map((r) => {
                ...
                const encerrado = r.meta.group === "encerrado";
                return (
                  <div key={r.key} className={`p-3 ${r.isComboGroup ? "bg-[#F7F7F8]" : ""}`}>
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-1.5">
                          ...
                          {r.clienteId ? (
                            <Link
                              to={`/clientes?cliente=${r.clienteId}`}
                              className="text-[13px] font-semibold text-[#2F3337] truncate hover:underline"
                            >
                              {r.clienteNome}
                            </Link>
                          ) : (
                    ...
                    {r.clienteId && (
                      <div className="mt-1.5 flex justify-end">
                        <Link
                          to={`/clientes?cliente=${r.clienteId}`}
                          className="shrink-0 inline-flex items-center gap-1 text-[11px] font-medium text-[#2F3337]"
                        >
    ...
    <section className="bg-white border border-slate-200 rounded-xl overflow-hidden shadow-sm flex flex-col">
      ...
      <div className="bg-slate-50 border-b border-slate-200 px-4 py-3 flex items-center justify-between">
        <div className="flex items-center gap-2">
          <span className={`size-2.5 rounded-full ${entidade === "PF" ? "bg-[#2F3337]" : "bg-emerald-700"}`} />
      ...
      <div className={`grid ${onlyAtivos ? "grid-cols-1" : "grid-cols-2 divide-x"} divide-slate-200 border-b border-slate-200`}>
        <div className="p-4">
          ...
          <div className="flex items-baseline gap-2 mt-1">
            <span className="text-3xl font-bold tabular-nums text-slate-900">{totals.ativos}</span>
            {homTotal > 0 && (
              <span
                className={`text-[10px] tabular-nums ${homAllDone ? "text-emerald-600" : "text-slate-400"}`}
                title="Homologados (validação visual local) sobre os ativos desta entidade"
              >
                {homOk}/{homTotal} homol. · {homPct}%
              </span>
            )}
          </div>
        </div>
  ...
  return (
    <div className="inline-flex flex-wrap items-center gap-1.5 p-2 rounded-lg border border-slate-300 bg-white shadow-sm">
      <select
        ...
        disabled={saving}
        className="h-7 text-[11px] rounded-md border border-slate-200 bg-white px-1.5 focus:outline-none focus:ring-2 focus:ring-[#2F3337] max-w-[180px]"
      >
      ...
      <input
        ...
        className="h-7 text-[11px] rounded-md border border-slate-200 bg-white px-1.5 focus:outline-none focus:ring-2 focus:ring-[#2F3337]"
      />
      <button
