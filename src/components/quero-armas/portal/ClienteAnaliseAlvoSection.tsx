  ...
  return (
    <section className="qa-alvo qa-client-summary-print">
      <style>{`
        ...
        .qa-alvo__head{display:flex;align-items:flex-start;justify-content:space-between;gap:16px;margin-bottom:14px}
        .qa-alvo__kicker{display:flex;align-items:center;gap:8px;font-size:10px;font-weight:900;letter-spacing:.24em;color:var(--muted);text-transform:uppercase;margin-bottom:8px}
        .qa-alvo__dot{width:7px;height:7px;border-radius:999px;background:var(--bordo)}
        .qa-alvo h1{font-family:Oswald,'Arial Narrow',Arial,sans-serif;font-weight:700;font-size:24px;line-height:1.05;margin:0;letter-spacing:.03em;color:var(--ink);text-transform:uppercase}
        .qa-alvo__lead{font-family:Arial,sans-serif;font-size:12px;line-height:1.4;color:var(--muted);margin:6px 0 0;max-width:640px}
        .qa-alvo__badge{display:inline-flex;align-items:center;gap:6px;border:1px solid var(--bordo);background:#fff;color:var(--bordo);font-family:Oswald,'Arial Narrow',Arial,sans-serif;font-size:10px;font-weight:900;letter-spacing:.22em;padding:6px 10px;border-radius:999px;text-transform:uppercase;white-space:nowrap}
        .qa-alvo__grid{display:grid;grid-template-columns:minmax(220px,300px) minmax(0,1fr);gap:16px;background:var(--card);border:1px solid var(--line);border-radius:3px;padding:16px;box-shadow:0 6px 14px rgba(17,17,17,.04)}
        .qa-alvo__target{position:relative;display:flex;align-items:center;justify-content:center;background:#fafafa;border:1px solid var(--line);border-radius:3px;padding:14px;min-height:260px}
        .qa-alvo__ring{position:relative;aspect-ratio:1/1;width:100%;max-width:240px;border-radius:999px;border:1px solid var(--line);background:radial-gradient(circle,#fff 0 36%,#f2f2f0 37% 39%,#fff 40% 54%,#f2f2f0 55% 57%,#fff 58% 100%)}
        ...
        .qa-alvo__hit{position:absolute;width:14px;height:14px;border-radius:999px;background:#fff;border:3px solid var(--bordo)}
        .qa-alvo__body{display:flex;flex-direction:column;gap:10px;min-width:0}
        .qa-alvo__h2{font-family:Georgia,'Times New Roman',serif;font-weight:700;font-size:17px;line-height:1.15;margin:0;color:#0c0c0c;letter-spacing:-.01em}
        .qa-alvo__p{font-family:Arial,sans-serif;font-size:12px;line-height:1.45;color:#4a4a4a;margin:0}
        .qa-alvo__cards{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px}
        .qa-alvo__card{border:1px solid var(--line);background:#fafafa;border-radius:3px;padding:8px 10px;display:grid;grid-template-columns:14px 1fr;gap:8px;align-items:start;min-height:60px}
        .qa-alvo__card svg{width:12px;height:12px;color:var(--ink);margin-top:2px}
        ...
        .qa-alvo__card-d{font-family:Arial,sans-serif;font-size:11px;line-height:1.32;color:var(--muted);margin-top:3px}
        .qa-alvo__note{border:1px solid rgba(122,31,43,.25);background:#fff;border-radius:3px;padding:10px 12px;margin-top:2px}
        ...
        .qa-alvo__note-p{font-family:Arial,sans-serif;font-size:11px;line-height:1.4;color:#4a4a4a;margin:0}
        .qa-alvo__note-legal{font-family:Arial,sans-serif;font-size:10px;line-height:1.35;color:#8a8a8a;margin:6px 0 0}
