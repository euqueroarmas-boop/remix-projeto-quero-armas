...
function stageTone(stage: DocUploadStage) {
  if (stage === "done") return "text-emerald-600";
  if (stage === "failed") return "text-red-500";
...
export default function ClientePecaAuxiliaryDocs({ userId, caseId, onChange }: Props) {
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          <p className="mt-1 text-[10px] leading-relaxed text-muted-foreground">
            BOs, laudos e demais provas são extraídos integralmente para compor o contexto da IA antes da geração.
          </p>
        </div>
      ...
      {docTotal > 0 && showList && (
        <div className="space-y-2">
          {items.map((item, index) => (
            <div
              key={`${item.nome}-${index}`}
              className="rounded-md border border-border/70 bg-card/60 px-3 py-2.5 space-y-2"
            >
