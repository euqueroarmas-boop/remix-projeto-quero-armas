...
- GCM (Guarda Civil Municipal / Guarda Municipal) entra em `seguranca_publica`. **Não há dispensa legal de exigências**: GCM cumpre o MESMO checklist do cidadão comum; a ÚNICA diferença é poder apresentar os laudos psicológico e de capacidade técnica emitidos pela própria instituição. Nunca marcar exames como "dispensado por lei" para segurança pública.
- Rótulos de tela (`statusDocumento.ts`): `dispensado` puro = "DISPENSADO POR LEI"; `nao_aplicavel` = "NÃO SE APLICA" (caminho não escolhido, ex.: laudo da instituição quando o cliente optou pelo particular); reaproveitamento = "REAPROVEITADO" (cliente: "JÁ RESOLVIDO").
